...
> 注意：飞书 OAuth 的权限范围由应用在开放平台上的配置决定，不由客户端控制。开通权限后需要发布新版本才会生效。如果是在用户已授权**之后**才新增的权限，该用户需要在 Quick Desktop 中断开并重新连接以触发再次授权，refresh token 不会自动获得新增 scope。
...
> Note: Feishu OAuth scopes are determined by the app's configuration on the Open Platform, not by the client. You must publish a new app version after enabling permissions for them to take effect. If permissions were added **after** a user already authorized, that user must disconnect and reconnect in Quick Desktop to re-authorize — refresh tokens do not automatically gain new scopes.
