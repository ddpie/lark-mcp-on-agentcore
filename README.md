...
- EventBridge 每 30 分钟刷新飞书 Token；调用 Feishu 前先做 SM 写探针，写不通就跳过本轮（refresh_token 不消耗）
...
| 调度 | EventBridge | 每 30 分钟触发 OAuth Lambda，扫描即将过期的飞书 token 并刷新 |
...
| 可观测 | CloudWatch Alarms × 10 | TokenLost / RefreshFailed / AgentCore5xx / McpLatencyP95 / FeishuNotAuth / Concurrency / Throttle / ApiGw5xx / OAuthError / MiddlewareError |
| 可观测 | SNS + Webhook Lambda | 告警 fan-out；部署时可配飞书群 Webhook，也可手动订阅邮箱/Slack/PagerDuty |
...
| 层面 | 说明 |
|------|------|
| 告警 | CloudWatch Alarm × 10（TokenLost / RefreshFailed / AgentCore5xx / McpLatencyP95 / FeishuNotAuth / Concurrency / Throttle / ApiGw5xx / Lambda Errors × 2）→ SNS |
| 看板 | 部署自动创建 CloudWatch Dashboard：请求量、延迟 P95、错误率、OAuth 漏斗、活跃用户 |
| 告警通知 | 部署时配置飞书群 Webhook URL，告警以消息卡片推送到群聊（支持中英文） |
| 日志 | 所有 Lambda 输出结构化 JSON；userId / open_id 以 sha256 前 16 位呈现；保留天数部署时可配 |
| 慢调用检测 | Feishu API 超 3s 输出 `feishu_slow`；AgentCore 超 5s 输出 `agentcore_slow` |
| 鉴权诊断 | Middleware 鉴权失败区分原因：`expired` / `signature_mismatch` / `malformed_payload` / `decode_error` |
| 埋点指标 | `McpRequestOk` / `McpLatencyMs` / `AuthFail` / `OAuthAuthorizeStart` / `OAuthCallbackSuccess` / `NewUserAuthorized` / `ActiveUsers` |

## 多语言

所有翻译集中在 `config/i18n.json`，新增语种只需加一个 key：

| 场景 | 语言来源 |
|------|---------|
| 部署脚本 | 部署时选择（zh/en） |
| CloudWatch 看板标题 | 跟随部署语言 |
| 告警卡片 | 跟随部署语言 |
| OAuth 成功页 | 跟随浏览器 Accept-Language |
| Fallback | 英文 |
...
| CloudWatch Alarms × 10 | $0.10/告警/月，合计 ~$1.00 |
| SSM Parameter Store (SecureString × 3) | ~$0.12/月 |
| ECR 镜像存储 | ~$0.10/GB/月（镜像约 600 MB） |
| WAFv2（可选，默认关） | $5/WebACL/月 + $1/规则/月 + $0.60/百万请求 |
| Lambda / API Gateway / CloudFront / EventBridge / SNS | 按请求量，有免费额度 |
...
A: 部署完成后会输出 Dashboard URL。也可以在 AWS CloudWatch 控制台搜索 `lark-mcp-on-agentcore` 看板。看板包含 5 个分区：流量、Lambda、OAuth/Token、基础设施、告警状态。
...
- EventBridge refreshes Feishu tokens every 30 minutes; SM write-probe ensures `refresh_token` is preserved when SM is unavailable
...
| Layer | Description |
|-------|-------------|
| Alarms | CloudWatch Alarms × 10 (TokenLost / RefreshFailed / AgentCore5xx / McpLatencyP95 / FeishuNotAuth / Concurrency / Throttle / ApiGw5xx / Lambda Errors × 2) → SNS |
| Dashboard | Auto-created CloudWatch Dashboard: request volume, P95 latency, error rate, OAuth funnel, active users |
| Alert notifications | Configure Feishu bot webhook URL at deploy time; alarms push interactive message cards (zh/en) |
| Logging | All Lambdas emit structured JSON; userId / open_id appear as sha256 first-16; retention configurable at deploy |
| Slow-call detection | Feishu API >3s emits `feishu_slow`; AgentCore >5s emits `agentcore_slow` |
| Auth diagnostics | Middleware token verification logs specific failure reason: `expired` / `signature_mismatch` / `malformed_payload` / `decode_error` |
| Metrics | `McpRequestOk` / `McpLatencyMs` / `AuthFail` / `OAuthAuthorizeStart` / `OAuthCallbackSuccess` / `NewUserAuthorized` / `ActiveUsers` |

### i18n

All translations are centralized in `config/i18n.json`. Add a new language by adding a single key:

| Context | Language source |
|---------|---------------|
| Deploy scripts | Selected at deploy time (zh/en) |
| CloudWatch dashboard titles | Follows deploy language |
| Alarm cards | Follows deploy language |
| OAuth success page | Follows browser Accept-Language |
| Fallback | English |
...
| CloudWatch Alarms × 10 | $0.10/alarm/month, ~$1.00 total |
| SSM Parameter Store (SecureString × 3) | ~$0.12/month |
| ECR image storage | ~$0.10/GB/month (image ~600 MB) |
| WAFv2 (optional, default off) | $5/WebACL/month + $1/rule/month + $0.60/M requests |
| Lambda / API Gateway / CloudFront / EventBridge / SNS | Per request, with free tier |
...
2. Group Settings (⚙️ top-right) → Group Bots → Add Bot
...
A: The deploy output includes the Dashboard URL. Or search for `lark-mcp-on-agentcore` in the AWS CloudWatch console. The dashboard has 5 sections: Traffic, Lambda, OAuth/Token, Infrastructure, and Alarm Status.
