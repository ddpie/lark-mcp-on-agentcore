...
lambda/
  token-refresh-shim/ OAuth 流程 + Token 自动刷新 + 增量授权
  mcp-middleware/     Token 验证 + SigV4 代理
scripts/
  deploy.sh           交互式部署 (中/英双语)
...
lambda/
  token-refresh-shim/ OAuth flow + token refresh + incremental auth
  mcp-middleware/     Token verification + SigV4 proxy
scripts/
  deploy.sh           Interactive deployment (Chinese/English)
