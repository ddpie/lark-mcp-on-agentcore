...
> 注意：飞书 OAuth 的权限范围由应用在开放平台上的配置决定，不由客户端控制。开通权限后需要发布新版本才会生效。
...
> Note: Feishu OAuth scopes are determined by the app's configuration on the Open Platform, not by the client. You must publish a new app version after enabling permissions for them to take effect.
