...
> 3. Do NOT default to `page_all=true` for time-bounded queries.
>
> Only use `page_all=true` from the beginning when:
...
lark_task_get_related_tasks()

# List incomplete related tasks starting from a page token
lark_task_get_related_tasks(include_complete="false", page_token="1752730590582902")

# Show only tasks created by me
lark_task_get_related_tasks(created_by_me=true)
```
...
|-----------|----------|-------------|
| `include_complete` | No | Default behavior includes completed tasks. Set to `"false"` to keep only incomplete tasks. |
| `page_all` | No | Automatically paginate through all pages (max 40). |
| `page_limit` | No | Max page limit (default 20). |
| `page_token` | No | Start from the specified page token. This token is the task's last update time cursor in microseconds. |
...
2. Execute `lark_task_get_related_tasks(...)`
