...
本文件覆盖 lark-slides 的通用创建前自检、XML 排障和常见失败处理。命令专属问题优先看对应 reference，例如 `lark_slides_replace_slide`、`lark_slides_media_upload`、`xml_presentation.slide.create`。
...
- 图片路径正确：`<img src="@...">` 只在 `lark_slides_create` 的 `slides` 参数链路中使用；直接调用 `xml_presentation.slide.create` 必须先拿到 `file_token`。
...
6. 如果使用 `slides` 参数一次性建多页失败，直接切到两步创建：先 `lark_slides_create`，再用 `xml_presentation.slide.create` 逐页添加。
7. 局部问题用 `lark_slides_replace_slide` 块级修正；整页结构要改时再用 `slide.delete` 旧页 + `slide.create` 新页。
...
- 原生 `slide.create` 包装、`before_slide_id` 细节：见 `lark_get_skill(domain="slides", section="xml-presentation-slide-create")`。
