...
可直接复制使用的 slide XML 模板。纯文本/形状模板可传给 `lark_invoke(tool_name="lark_slides_xml_presentation_slide_create", ...)`：

```
lark_invoke(tool_name="lark_slides_xml_presentation_slide_create", args={
  params: {"xml_presentation_id": "YOUR_ID"},
  data: {"slide": {"content": "PASTE_XML_HERE"}}
})
```

> **带图模板不要直接提交。** 新建 PPT 时可在 `lark_slides_create(slides='[...]')` 中使用 `src="@./local.png"`，会自动上传并替换为 `file_token`；给已有 PPT 添加或修改图片时，必须先用 `lark_slides_media_upload` 拿到 `file_token`，再写进 `<img src="...">`。
