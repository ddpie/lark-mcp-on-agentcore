...
`iconType` 必须来自已验证的 IconPark 路径。需要语义图标时，先调用 `lark_exec_script(script="lark-slides/scripts/iconpark_tool.py", args=["search", "--query", "<语义>"])`，不要凭记忆拼路径。更多规则见 `lark_get_skill(domain="slides", section="iconpark")`。
