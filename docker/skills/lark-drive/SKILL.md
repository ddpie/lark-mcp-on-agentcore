---
name: lark-drive
description: "飞书云空间（云盘/云存储）：管理云空间中的文件和文件夹。上传和下载文件、创建文件夹、复制/移动/删除文件、查看文件元数据、管理文档评论、管理文档权限、订阅用户评论变更事件、修改文件标题（docx、sheet、bitable、file、folder、wiki）；也负责把本地 Word/Markdown/Excel/CSV/PPTX 以及 Base 快照（.base）导入为飞书在线云文档（docx、sheet、bitable、slides）。当用户需要上传或下载文件、整理云空间目录、查看文件详情、管理评论、管理文档权限、修改文件标题、订阅用户评论变更事件，或要把本地文件导入成新版文档、电子表格、多维表格/Base/幻灯片时使用。\"云空间\"、\"云盘\"和\"云存储\"是同一概念，用户说\"云盘\"、\"云存储\"、\"网盘\"、\"我的空间\"时均路由到本 skill。当用户给出 doubao.com 的云空间资源 URL/token，或明确提到豆包里的 file/folder/docx/sheet/bitable/wiki 资源时，也应直接使用本 skill，不要因为域名不是飞书而回退到 WebFetch；路由依据是资源类型、URL 路径模式和 token，而不是域名。"
---
...
- 用户要**搜文档 / Wiki / 电子表格 / 多维表格 / 云空间（云盘/云存储）对象**，优先使用 `lark_drive_search()`。自然语言里"最近我编辑过的"、"我创建的"（→ `mine=true`，实为 owner 语义）、"最近一周我打开过的 xxx"、"某人 owner 的 docx" 等直接映射到扁平参数，避免手写嵌套 JSON。
...
**推荐方式：使用 `lark_drive_inspect` 自动解包**
...
- `lark_drive_add_comment` 支持两种模式。
- 全文评论：未传 `block_id` 时默认启用，也可显式传 `full_comment=true`；支持 `docx`、旧版 `doc` URL、白名单扩展名的 Drive file，以及最终解析为 `doc`/`docx`/`file` 的 wiki URL。
- 局部评论：传 `block_id` 时启用；`docx` 支持文本定位或 block id，`sheet` 支持 `<sheetId>!<cell>`，`slides` 支持 `<slide-block-type>!<xml-id>`，wiki URL 解析到这些类型时也支持对应局部评论。Drive file 本次只支持全文评论，不支持局部评论。
- Drive file 评论仅支持白名单扩展名：`.md`、`.txt`、`.json`、`.csv`、`.go`、`.js`、`.py`、`.pptx`、`.png`、`.jpg`、`.jpeg`、`.zip`、`.mp3`、`.mp4`。`.pdf`、`.docx`、`.xlsx` 等未在白名单内的普通文件暂不支持，工具会直接报错提示当前还不支持这种类型的评论。
- Review / 审阅 / 校对 / 逐条指出问题场景优先使用局部评论，不要把多个可定位问题汇总成一条全文评论；具体参数和定位方式见 `lark_get_skill(domain="drive", section="add-comment")` 行为说明。
...
- 如果 wiki 解析后不是 `doc`/`docx`/`file`/`sheet`/`slides`，不要用 `lark_drive_add_comment`。
- 如果需要更底层地直接调用评论 V2 协议，再走原生 API：先执行 `lark_discover(query="drive.file.comments.create_v2")`，再执行 `lark_invoke(tool_name="lark_drive_file_comments_create_v2", ...)`。全文评论省略 `anchor`，局部评论传 `anchor.block_id`。

### 评论查询与统计口径（关键！）

**强制规则**：`lark_invoke(tool_name="lark_drive_file_comments_list", ...)` 默认必须传 `is_solved:false`，即仅查询未解决评论。即使用户说"所有评论""全部评论""把评论都列出来"，只要没有明确提到要包含已解决评论，仍然按默认口径查询未解决评论。仅当用户明确要求包含已解决评论时，才可省略 `is_solved` 参数。
...
- 查询文档评论时，使用 `lark_invoke(tool_name="lark_drive_file_comments_list", ...)`。
- 工具返回的 `items` 应理解为"评论卡片"列表，每个 `item` 对应用户界面里看到的一张评论卡片，而不是平铺的互动消息列表。
...
- 如果某个 `item.has_more=true`，说明该评论卡片下还有更多回复未包含在当前返回中；此时需要继续调用 `lark_invoke(tool_name="lark_drive_file_comment_replys_list", ...)` 拉全后，再做全量回复数 / 总互动数统计。
...
- 具体参数、定位方式和不同文档类型的约束见 `lark_get_skill(domain="drive", section="add-comment")` 行为说明。
...
- 如果用户只说"第一条评论"，直接使用工具返回的第一条即可，不需要额外排序。
...
#### 批量查询与列表查询的选择
- 使用 `lark_invoke(tool_name="lark_drive_file_comments_batch_query", ...)` 是**已知评论 ID 后**的批量查询，需要传入具体的评论 ID 列表。
- 使用 `lark_invoke(tool_name="lark_drive_file_comments_list", ...)` 用于分页获取评论列表，适合统计评论总数、遍历所有评论，或获取"最新/最后 N 条评论"等场景。

#### 评论定位字段
- 需要根据评论定位到文档正文位置时（例如根据评论 review 文档、区分多处相同引用文本、把评论落点映射到 `lark_docs_fetch` 的 block），先确认目标是 `file_type=docx`，再调用 `lark_get_skill(domain="drive", section="comment-location")` 查看评论定位字段说明，其他文档类型暂不支持返回定位字段。

#### Reaction / 表情场景
- 遇到评论 / 回复上的 reaction（表情、各表情数量、谁点了什么、添加/删除表情）相关问题时，**先调用 `lark_get_skill(domain="drive", section="reactions")` 了解如何使用**。
...
调用 `lark_invoke(tool_name="lark_drive_permission_public_patch", ...)` 更新文档公开权限失败时，如果返回以下错误码，按表格给用户明确下一步。不要把这些错误简单归类为缺少 scope；它们通常表示租户、对外分享或文档密级策略拦截。

| 错误码 | 含义                     | 给用户的引导 |
|--------|------------------------|--------------|
| `91009` | 对外分享被租户安全策略管控，当前用户无法开启 | 提示用户：对外分享能力被租户安全策略统一管控，无法通过 API 或当前用户直接开启；需要联系租户管理员调整组织级对外分享策略。 |
| `91010` | 文档对外分享未打开              | 提示用户：当前文档尚未打开对外分享，请先在文档权限设置中打开对外分享，再重试。 |
...
当需要将文档权限授予**当前应用（bot）自身**时，需要通过 bot info 接口获取应用的 open_id，再调用权限接口授权。此操作需要 bot identity (`tenant_access_token`)，MCP server 仅支持 user identity。
...
| Shortcut | 说明                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
|----------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `lark_get_skill(domain="drive", section="search")` | Search Lark docs, Wiki, and spreadsheet files with flat filter flags. Natural-language-friendly: `edited_since`, `mine`, `doc_types`, etc.                                                                                                                                                                                                                                                                                                                                                                                   |
| `lark_get_skill(domain="drive", section="upload")` | Upload a local file to a Drive folder or wiki node                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| `lark_get_skill(domain="drive", section="create-folder")` | Create a Drive folder, optionally under a parent folder                                                                                                                                                                                                                                                                                                                                                                               |
| `lark_get_skill(domain="drive", section="download")` | Download a file from Drive to local                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| `lark_get_skill(domain="drive", section="preview")` | List or download available preview artifacts for a Drive file; explicit `type` required for downloads |
| `lark_get_skill(domain="drive", section="cover")` | List or download stable built-in cover presets for a Drive file; download-time HTTP 404 means the file has no artifact for that cover spec |
| `lark_get_skill(domain="drive", section="status")` | Compare a local directory with a Drive folder by exact SHA-256 hash by default, or use `quick=true` for a best-effort modified-time diff that skips remote downloads; reports `new_local` / `new_remote` / `modified` / `unchanged` plus `detection=exact` or `detection=quick`. Duplicate remote `rel_path` conflicts fail fast with `error.type=duplicate_remote_path` and list every conflicting entry; do not proceed as if one was chosen. `local_dir` 必须是 cwd 内的相对路径，越界路径工具会直接拒绝；目标在 cwd 外时引导用户切换 agent 工作目录，不要私自 `cd` 绕过。 |
| `lark_get_skill(domain="drive", section="pull")` | File-level Drive → local mirror. Duplicate remote `rel_path` conflicts fail by default; for duplicate files, `rename` downloads all copies with stable hashed suffixes, while `newest` / `oldest` pick one. `if_exists` supports `overwrite` / `smart` / `skip` (`smart` is a best-effort modified-time incremental mode for repeat syncs). `delete_local` requires `_confirm=true`, only removes regular files, and is skipped after item failures. `local_dir` must stay inside cwd.                                               |
...
| `lark_get_skill(domain="drive", section="create-shortcut")` | Create a shortcut to an existing Drive file in another folder                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| `lark_get_skill(domain="drive", section="add-comment")` | Add a comment to doc/docx/file/sheet/slides, also supports wiki URL resolving to doc/docx/file/sheet/slides; file targets support selected extensions and full comments only                                                                                                                                                                                                                                                                                                                                                       |
| `lark_get_skill(domain="drive", section="export")` | Export a doc/docx/sheet/bitable/slides to a local file with limited polling; supports `file_name` for local naming                                                                                                                                                                                                                                                                                                               |
| `lark_get_skill(domain="drive", section="export-download")` | Download an exported file by file_token                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| `lark_get_skill(domain="drive", section="import")` | Import a local file to Drive as a cloud document (docx, sheet, bitable, slides)                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| `lark_get_skill(domain="drive", section="version-history")` | List historical versions of a file with only_tag=true and cursor-based pagination                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| `lark_get_skill(domain="drive", section="version-get")` | Download a specific historical version of a file                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| `lark_get_skill(domain="drive", section="version-revert")` | Revert a file to a specific historical version                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `lark_get_skill(domain="drive", section="version-delete")` | Delete a specific historical version of a file                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `lark_get_skill(domain="drive", section="move")` | Move a file or folder to another location in Drive                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| `lark_get_skill(domain="drive", section="delete")` | Delete a Drive file or folder with limited polling for folder deletes                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| `lark_get_skill(domain="drive", section="push")` | File-level local → Drive mirror. Duplicate remote `rel_path` conflicts fail by default; `newest` / `oldest` only apply to duplicate files when you explicitly want to target one remote file. `if_exists` supports `skip` / `smart` / `overwrite` (`smart` skips files whose remote `modified_time` is already up to date, but falls through to the same overwrite path when the remote is older, so it inherits overwrite's rollout caveat). `delete_remote` requires `_confirm=true`. `local_dir` must stay inside cwd.            |
| `lark_get_skill(domain="drive", section="task-result")` | Poll async task result for import, export, move, or delete operations                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| `lark_get_skill(domain="drive", section="inspect")` | Inspect a Lark document URL to get its type, title, and canonical token; auto-unwraps wiki URLs to the underlying document                                                                                                                                                                                                                                                                                                                                                                                                         |
| `lark_get_skill(domain="drive", section="apply-permission")` | Apply to the document owner for view/edit access (user-only; 5/day per document)                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| `lark_get_skill(domain="drive", section="secure-label")` | List secure labels available to the current user |
| `lark_get_skill(domain="drive", section="secure-label")` | Update a Drive file/document secure label; downgrade approval errors require opening the document UI |
...
> `quota_details.get` 是 user-only OpenAPI：`quota_detail_id` 应填写当前用户的 `user_id`。
