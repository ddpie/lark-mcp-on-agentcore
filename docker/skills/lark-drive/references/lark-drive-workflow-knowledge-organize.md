...
This file implements the registered knowledge organization workflow. Before execution, the agent MUST read `lark_get_skill(domain="drive", section="workflow")` and follow the shared execution protocol, Artifact Contract, Workflow Loading rules, and write confirmation rules (authentication is handled automatically by the MCP server).
...
- Wiki / personal library target: `lark_get_skill(domain="wiki")`
- Content read required: `lark_get_skill(domain="doc")` and `lark_get_skill(domain="doc", section="fetch")`
- Sheet down-drill required: `lark_get_skill(domain="sheets")`
- Base down-drill required: `lark_get_skill(domain="base")`
...
|------|------|--------------|
| `workflow-knowledge-organize` | Workflow trigger, global contract, state machine, progressive load map, command family allowlist | Stage-specific rules, output templates, execution details |
| `workflow-knowledge-organize-discovery` | `PARSE_SCOPE`, `INVENTORY`, target parsing, stop conditions, inventory limits, `ResourceItem` | Classification, plan generation, write execution |
| `workflow-knowledge-organize-analysis` | `CONTENT_READ`, `ISSUE_ANALYSIS`, `RULE_GENERATION`, low-confidence reads, issue rules, problem pagination, classification, target tree | Plan execution, write confirmation, verification |
| `workflow-knowledge-organize-planning` | `PLAN_GENERATION`, `EXEC_CONFIRM`, `PlanItem`, `DisplayItem`, plan pagination, plan revision, execution scope confirmation | Scope parsing, resource inventory, write execution, verification |
| `workflow-knowledge-organize-execution` | `EXECUTE`, `VERIFY`, `PathTokenMap`, write execution, progress reporting, verification, next suggestions, internal recovery hooks | Scope parsing, resource inventory, classification, plan generation, plan revision, rollback execution details |
| `workflow-knowledge-organize-rollback` | `ROLLBACK_CONFIRM`, `ROLLBACK`, `ROLLBACK_VERIFY`, `ROLLBACK_CLEANUP_CONFIRM`, `ROLLBACK_CLEANUP`, `ROLLBACK_CLEANUP_VERIFY`, recovery plan generation, recovery execution, cleanup verification | Scope parsing, resource inventory, classification, organization plan generation, normal write execution |
...
Agent MUST load the phase file for the active state before executing that state.

| State | Required Phase File |
|-------|---------------------|
| `PARSE_SCOPE` | `lark_get_skill(domain="drive", section="workflow-knowledge-organize-discovery")` |
| `INVENTORY` | `lark_get_skill(domain="drive", section="workflow-knowledge-organize-discovery")` |
| `CONTENT_READ` | `lark_get_skill(domain="drive", section="workflow-knowledge-organize-analysis")` |
| `ISSUE_ANALYSIS` | `lark_get_skill(domain="drive", section="workflow-knowledge-organize-analysis")` |
| `RULE_GENERATION` | `lark_get_skill(domain="drive", section="workflow-knowledge-organize-analysis")` |
| `PLAN_GENERATION` | `lark_get_skill(domain="drive", section="workflow-knowledge-organize-planning")` |
| `EXEC_CONFIRM` | `lark_get_skill(domain="drive", section="workflow-knowledge-organize-planning")` |
| `EXECUTE` | `lark_get_skill(domain="drive", section="workflow-knowledge-organize-execution")` |
| `VERIFY` | `lark_get_skill(domain="drive", section="workflow-knowledge-organize-execution")` |
| `ROLLBACK_CONFIRM` | `lark_get_skill(domain="drive", section="workflow-knowledge-organize-rollback")` |
| `ROLLBACK` | `lark_get_skill(domain="drive", section="workflow-knowledge-organize-rollback")` |
| `ROLLBACK_VERIFY` | `lark_get_skill(domain="drive", section="workflow-knowledge-organize-rollback")` |
| `ROLLBACK_CLEANUP_CONFIRM` | `lark_get_skill(domain="drive", section="workflow-knowledge-organize-rollback")` |
| `ROLLBACK_CLEANUP` | `lark_get_skill(domain="drive", section="workflow-knowledge-organize-rollback")` |
| `ROLLBACK_CLEANUP_VERIFY` | `lark_get_skill(domain="drive", section="workflow-knowledge-organize-rollback")` |

## Command Map

Use only command families allowed for the current state. Detailed syntax belongs to referenced skills / shortcut docs.

| State | Allowed Tools | Purpose |
|-------|---------------|---------|
...
| `EXEC_CONFIRM` | No write tools | Ask user to confirm execution scope |
| `EXECUTE` | `lark_drive_create_folder`, `lark_drive_move`, `lark_wiki_node_create`, `lark_wiki_move` existing-node mode only, `lark_drive_task_result`, `lark_drive_apply_permission` only when explicitly confirmed | Execute whitelisted writes |
| `VERIFY` | `lark_wiki_node_list`, `lark_invoke(tool_name="lark_drive_files_list", ...)`, `lark_drive_task_result` if async result remains pending | Verify actual result |
| `ROLLBACK_CONFIRM` | No write tools | Generate internal recovery plan and ask for restore confirmation |
| `ROLLBACK` | `lark_drive_move`, `lark_wiki_move` existing-node mode only, `lark_drive_task_result` | Execute confirmed reverse moves |
| `ROLLBACK_VERIFY` | `lark_wiki_node_list`, `lark_invoke(tool_name="lark_drive_files_list", ...)`, `lark_drive_task_result` if async result remains pending | Verify recovery result |
| `ROLLBACK_CLEANUP_CONFIRM` | No write tools | Generate cleanup plan and ask for delete confirmation |
| `ROLLBACK_CLEANUP` | `lark_drive_delete`, `lark_wiki_node_delete`, `lark_drive_task_result` | Delete only confirmed workflow-created safe-empty folders / nodes |
| `ROLLBACK_CLEANUP_VERIFY` | `lark_wiki_node_list`, `lark_invoke(tool_name="lark_drive_files_list", ...)`, `lark_drive_task_result` if async result remains pending | Verify cleanup deletion result |

## Wiki Move Mode Constraint

This workflow MUST NOT use `lark_wiki_move` docs-to-wiki mode. Wiki moves MUST use existing Wiki node mode with `node_token` only.

## Permission Request Gate

`lark_drive_apply_permission` is a write operation and may notify the resource owner. If any state hits resource access denial:
...
2. If auth or API scope is missing, stop and report the missing scope.
...
- Discovery phase: `lark_get_skill(domain="drive", section="workflow-knowledge-organize-discovery")`
- Analysis phase: `lark_get_skill(domain="drive", section="workflow-knowledge-organize-analysis")`
- Planning phase: `lark_get_skill(domain="drive", section="workflow-knowledge-organize-planning")`
- Execution phase: `lark_get_skill(domain="drive", section="workflow-knowledge-organize-execution")`
- Rollback phase: `lark_get_skill(domain="drive", section="workflow-knowledge-organize-rollback")`
- `lark_get_skill(domain="drive")` — main drive skill
- `lark_get_skill(domain="drive", section="files-list")`
- `lark_get_skill(domain="drive", section="search")`
- `lark_get_skill(domain="drive", section="inspect")`
- `lark_get_skill(domain="drive", section="apply-permission")`
- `lark_get_skill(domain="drive", section="task-result")`
- `lark_get_skill(domain="drive", section="delete")`
- `lark_get_skill(domain="wiki")`
- `lark_get_skill(domain="wiki", section="node-delete")`
- `lark_get_skill(domain="doc")`
- `lark_get_skill(domain="doc", section="fetch")`
- `lark_get_skill(domain="sheets")`
- `lark_get_skill(domain="base")`
