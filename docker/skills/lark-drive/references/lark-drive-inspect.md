...
```
# 检视一个 docx URL
lark_drive_inspect(url="https://xxx.feishu.cn/docx/doxcnXXX")

# 检视一个 wiki URL（自动解包到底层文档）
lark_drive_inspect(url="https://xxx.feishu.cn/wiki/wikcnXXX")

# bare token 需要指定 type
lark_drive_inspect(url="doxcnXXX", type="docx")

# 格式化输出
lark_drive_inspect(url="https://xxx.feishu.cn/base/bascnXXX", format="pretty")
```
...
| 场景 | 命令 |
|------|------|
| 用户给了一个 URL，想知道它是什么类型的文档 | `lark_drive_inspect(url="<url>")` |
| wiki 链接需要拿到底层文档的 token 来做后续操作 | `lark_drive_inspect(url="<wiki_url>")`，取输出中的 `token` |
| 只有 token 没有 URL | `lark_drive_inspect(url="<token>", type="<type>")` |
...
- `lark_drive_inspect()` 只用于识别/消歧；如果任务已能通过 URL 路径形态完成路由判断，不必把它作为所有 Drive 操作的通用前置步骤
- `lark_drive_inspect()` 失败后不要自动切到写接口继续尝试，先按错误提示处理权限、scope 或链接问题
