...
- `lark_drive_add_comment()` 支持全文评论和局部评论。
- 全文评论：未传 `block_id` 时默认启用，也可显式传 `full_comment=true`；支持 `docx`、旧版 `doc` URL、白名单扩展名的 Drive file，以及最终解析为 `doc` / `docx` / `file` 的 wiki URL。
- 局部评论：传 `block_id` 时启用；`docx` 支持文本定位或 block id，`sheet` 支持 `<sheetId>!<cell>`，`slides` 支持 `<slide-block-type>!<xml-id>`，wiki URL 解析到这些类型时也支持对应局部评论。
...
- 评论写入内容里的文本不能直接出现 `<`、`>`；提交前应转义为 `&lt;`、`&gt;`。`lark_drive_add_comment()` 会对 `type=text` 文本元素自动兜底转义；直接调用原生评论 API 时需要自行转义。
...
- `lark_invoke(tool_name="lark_drive_file_comments_list", ...)` 返回的 `items` 是评论卡片列表，每个 `item` 对应用户界面中的一张评论卡片，不是平铺的互动消息列表。
...
- 如果 `item.has_more=true`，说明该评论卡片下还有更多回复未包含在当前返回中；需要继续调用 `lark_invoke(tool_name="lark_drive_file_comment_replys_list", ...)` 拉全后，再做全量回复数或总互动数统计。
...
- 用户只说"第一条评论"时，直接使用 `lark_invoke(tool_name="lark_drive_file_comments_list", ...)` 返回的第一条，不需要额外排序。
...
需要更底层地直接调用评论 V2 协议时，先查看参数结构，再调用原生命令。全文评论省略 `anchor`，局部评论传 `anchor.block_id`。

```
lark_discover(query="drive.file.comments.create_v2")
lark_invoke(tool_name="lark_drive_file_comments_create_v2", args={
