...
It MUST NOT generate organization plans, revise classification rules, execute unconfirmed deletes, rename resources, modify permissions, or use `lark_wiki_move` docs-to-wiki mode.
...
| `input_token` | Token supplied to the tool |
| `input_node_token` | Wiki node token supplied to the tool |
| `input_parent_token` | Source parent token when known |
| `target_parent_token` | Target parent token supplied to the tool |
| `returned_token` | Token returned by the tool |
| `returned_node_token` | Wiki node token returned by the tool |
| `returned_parent_token` | Parent token returned by the tool |
| `task_id` | Async task ID when returned |
| `next_command` | Async continuation info when returned |
...
5. Async operations are `pending` until `lark_drive_task_result` proves completion.
...
Use only these tool forms:

```
# Drive resource back to original parent folder
lark_drive_move(file_token="<current_token>", type="<type>", folder_token="<original_parent_token>")

# Drive resource back to root
lark_drive_move(file_token="<current_token>", type="<type>")

# Wiki node back to original parent node
lark_wiki_move(node_token="<current_node_token>", target_parent_token="<original_parent_token>")

# Wiki node back to original space root
lark_wiki_move(node_token="<current_node_token>", target_space_id="<original_space_id>")
```

MUST NOT:

- Use `lark_wiki_move` docs-to-wiki mode.
...
3. Continue async move tasks with `lark_drive_task_result` when needed.
...
4. Continue async delete tasks with `lark_drive_task_result` when needed.
...
lark_wiki_node_delete(node_token="<node_token>", obj_type="wiki", include_children=true, _confirm=true)
```

`_confirm=true` is allowed only after the user explicitly confirmed cleanup deletion.
