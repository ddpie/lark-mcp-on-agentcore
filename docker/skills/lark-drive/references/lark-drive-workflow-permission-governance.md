...
不要声称已完成协作者列表验证：当前工具 surface 没有 `permission.members list` shortcut。
...
| `FACT_READ` | `lark_discover(query="drive.metas.batch_query")`；涉及公开权限时再调用 `lark_discover(query="drive.permission.public.get")`；涉及活跃度、访问复核或生命周期判断时再调用 `lark_discover(query="drive.file.statistics.get")` 和 `lark_discover(query="drive.file.view_records.list")` |
...
|-------|--------------------------|---------|
| `TARGET_INSPECT` | `lark_drive_inspect` | 解析 URL、type、canonical token、title 和 wiki unwrap data |
...
| `FACT_READ` | `lark_invoke(tool_name="lark_drive_permission_public_get")` | 读取支持类型的文档公共访问和协作权限设置，包括链接分享、对外分享、协作者管理、复制内容、创建副本、打印、下载和评论 |
...
| `EXEC_CONFIRM` | `lark_invoke(tool_name="lark_drive_permission_members_auth")` | 文档公共访问和协作权限设置修改前检查 `action=manage_public` |
...
| `VERIFY` | `lark_invoke(tool_name="lark_drive_metas_batch_query")`, `lark_invoke(tool_name="lark_drive_permission_public_get")` | 验证支持的 metadata，包括 owner、secure-label 和文档公共访问与协作权限设置变更；权限申请只能表述为已发起 |
...
6. `permission_public` 用户可见含义是"文档公共访问和协作权限设置"，语义以官方 OpenAPI 字段说明为准，同时兼容当前工具 schema 返回的字段：优先使用 `external_access_entity`，缺失时才用 `external_access` boolean 映射为 `open` / `closed`；`manage_collaborator_entity`、`copy_entity`、`lock_switch` 等字段缺失时标记为 unknown，不要伪造；未识别字段保留在 raw evidence / partial note 中。
...
每个可审计目标都必须先归一化为 `per_target_permission_assessment`，再按 `lark_get_skill(domain="drive", section="workflow-permission-governance-outputs")` 的 `Semantic Rendering` 渲染。`public_exposure_check` 只是 `target_count=1` 的轻量渲染模式；它和多目标、容器诊断复用同一套语义字段与风险分类。该判断只覆盖当前文档公共访问和协作权限设置，不审计协作者名单、历史权限变更、完整继承链或审计日志。
...
- `drive permission.public get` 只用于 `lark_drive_inspect` 或 `DISCOVER_TARGETS` 可解析且运行时 schema 支持的目标类型；类型集合不要硬编码，执行时以 `lark_discover(query="drive.permission.public.get")` 为准。
...
- 当前工具没有 `permission.members list`、完整继承链、DLP 扫描、AI 索引状态、审计日志和跨平台权限事实。遇到这些需求必须记录为 `unsupported_checks` 或建议新增独立 workflow。
