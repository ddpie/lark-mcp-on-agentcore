...
列出 doc/docx/sheet/file/slides/base(bitable) 的评论卡片。优先传用户给出的完整 URL，`lark_drive_list_comments()` 会自动识别类型；如果传 wiki URL 或 `token="<wiki_token>", type="wiki"`，会先解析到真实文档。
...
| `type` | 裸 token 时必填 | 传 token 对应类型：`doc`、`docx`、`sheet`、`file`、`slides`、`bitable`、`base`、`wiki`。wiki token 使用 `wiki`；传 `base` 时，会按 `bitable` 类型处理。 |
