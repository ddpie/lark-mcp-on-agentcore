...
1. `active_plan_items` and `execution_scope` MUST already exist from `lark_get_skill(domain="drive", section="workflow-knowledge-organize-planning")`.
...
- 创建 Drive 文件夹：`lark_drive_create_folder`
- 移动 Drive 文件 / 文件夹：`lark_drive_move`
- 创建 Wiki 节点：`lark_wiki_node_create`
- 移动已有 Wiki 节点：`lark_wiki_move` (with `node_token`)
- 续跑异步移动任务：`lark_drive_task_result`
- 单个资源权限申请：`lark_drive_apply_permission`
...
6. Before writing to `my_library` root, resolve the real `space_id` according to `lark_get_skill(domain="wiki")`.
...
| Async move returns `ready=false` or `next_command` | Follow the returned async continuation with `lark_drive_task_result` | Do not assume completion |
