...
如果 Drive 中多个条目映射到同一个 `rel_path`，`lark_drive_status` 会在下载/hash 前直接失败，返回 `error.type=duplicate_remote_path`，并在 `error.detail.duplicates_remote[]` 中列出该路径下所有冲突条目的 `file_token`、`type`、名称、大小和时间字段；其中 `created_time`、`modified_time` 缺失时会省略，`size` 在缺失或为 `0` 时都可能被省略。不要把这种情况当成普通 `modified`；它表示同步域本身有歧义，需要先整理云端结构，或在 `lark_drive_pull` / `lark_drive_push` 中仅对"duplicate file"场景显式选择冲突策略。
