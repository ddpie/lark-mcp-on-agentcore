...
1. `resource_items` MUST already exist from `lark_get_skill(domain="drive", section="workflow-knowledge-organize-discovery")`.
2. For document partial reads, follow `lark_get_skill(domain="doc")` and `lark_get_skill(domain="doc", section="fetch")`.
3. For sheet / bitable down-drill, follow `lark_get_skill(domain="sheets")` or `lark_get_skill(domain="base")` only when title and path are insufficient.
