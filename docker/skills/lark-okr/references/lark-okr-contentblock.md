...
OKR shortcut 工具支持 `style` 参数控制 content/notes 字段的输入输出格式：
...
**重要**：输入时严格根据 `style` 值验证格式，不会自动检测。输出时读操作（如 `lark_okr_cycle_detail`、`lark_okr_progress_get`）根据 `style` 返回对应格式。
...
> **如何获取 `file_token`？** 使用 `lark_okr_upload_image` 工具上传本地图片，返回的 `file_token` 可用于构建 `ContentGallery` 图片块。
...
使用方式：
```
lark_okr_patch(level="objective", style="simple", target_id="123", content='{"text":"提升用户满意度","mention":["ou_123"]}')
```
