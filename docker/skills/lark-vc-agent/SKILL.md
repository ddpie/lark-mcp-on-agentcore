...
| "帮我参会，结束后把纪要发到群" 等跨阶段场景                                    | 按序编排：本 skill（入会 → 读事件）→ 会议结束后用 `lark_get_skill(domain="vc")` / `lark_get_skill(domain="minutes")` 拉纪要 → `lark_get_skill(domain="im")` 发群 |
...
1. 只有用户明确要求机器人退出 / 离开 / 结束参会时，才用 `lark_vc_meeting_leave(meeting_id="<从 lark_vc_meeting_join 拿到的 meeting.id>")`；不要把任务完成当作离会指令。
...
如果用户随后明确要求退出 / 离开 / 结束参会，再单独调用 `lark_vc_meeting_leave(meeting_id="<MID>")`。
