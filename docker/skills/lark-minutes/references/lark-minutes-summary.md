...
替换前建议先用 `lark_vc_notes(minute_tokens="<token>")` 读取当前总结，确认 `minute_token` 与待替换内容无误。
...
| 会议产物查询 | `lark_vc_notes(minute_tokens="<token>")` |
...
- lark-vc 的 `lark_vc_notes` — 读取总结、待办等 AI 产物
