...
> **注意**：`lark_task_get_my_tasks` 不带 `complete` 时会**同时返回已完成和未完成任务**，会把已完成任务当成"待办"展示进摘要里。站会/日报这种 pending 汇总场景**必须**显式带上 `complete="false"`，不要省略。
>
> 数据量层面也建议加过滤：
> - 用 `due_end` 过滤出目标日期前到期的任务
> - 如果也需要无截止日期的任务，可不加 `due_end`，但 AI 汇总时只展示**近 30 天内创建的**，其余折叠为"其他 N 项历史待办"
...
| 工具 | 所需 scope |
|------|-----------|
| `lark_calendar_agenda` | `calendar:calendar.event:read` |
| `lark_task_get_my_tasks` | `task:task:read` |
