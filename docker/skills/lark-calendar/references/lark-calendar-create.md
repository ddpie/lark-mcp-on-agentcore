...
# 第一步：创建日程（含高级参数）
## 查看完整参数定义
lark_discover(query="calendar.events.create")
## 创建日程
lark_invoke(tool_name="lark_calendar_events_create", args={
  params: {"calendar_id": "<CALENDAR_ID>"},
  data: {"summary": "技术分享：CLI 架构设计", "start_time": {"timestamp": "1741586400"}, "end_time": {"timestamp": "1741593600"}}
})

# 第二步：添加参会人（使用第一步返回的 calendar_id 和 event_id）
## 查看完整参数定义
lark_discover(query="calendar.event.attendees.create")
## 添加参会人
...
## 查看完整参数定义
lark_discover(query="calendar.events.delete")
## 删除空日程
lark_invoke(tool_name="lark_calendar_events_delete", args={
