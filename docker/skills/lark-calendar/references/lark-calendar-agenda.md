...
# 自定义时间范围（ISO 8601）
lark_calendar_agenda(start="2026-03-10T00:00+08:00", end="2026-03-17T00:00+08:00")

# 自定义时间范围（仅日期）
lark_calendar_agenda(start="2026-03-10", end="2026-03-17")

# 人类可读格式输出
lark_calendar_agenda(format="pretty")

# 指定日历
lark_calendar_agenda(calendar_id="cal_xxx")
```
...
- 查看多个日历：先用 `lark_invoke(tool_name="lark_calendar_calendars_list", args={page_all: true})` 列出日历列表，再逐个查询。
