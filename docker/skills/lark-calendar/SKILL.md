...
**CRITICAL — 凡涉及预约日程/会议或查询/搜索会议室，第一步 MUST 调用 lark_get_skill(domain="calendar", section="schedule-meeting")。禁止跳过此步直接调用任何 MCP tool！**
...
| `lark_calendar_search_event` | 按关键词、时间范围和参会人搜索日程, 仅返回 日程ID/主题/时间等信息，详情需走 `lark_invoke(tool_name="lark_calendar_events_get")` |
...
| `lark_calendar_freebusy` | 查询用户主日历的忙闲信息和 RSVP 状态 |
...
| 场景 | 前置要求 |
|------|----------|
| 预约日程/会议、查会议室 | 先调用 lark_get_skill(domain="calendar", section="schedule-meeting") |
...
| 预约/改约日程、添加/移除参会人、添加/更换会议室、调整时间 | 先判断新建 vs 编辑，再进入 lark_get_skill(domain="calendar", section="schedule-meeting") 工作流 |
...
- 凡是"预定/查询/搜索可用会议室"，都必须进入 lark_get_skill(domain="calendar", section="schedule-meeting") 工作流处理。
...
```
lark_invoke(tool_name="lark_calendar_<resource>_<method>", args={...})
```
