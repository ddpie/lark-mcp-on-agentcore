...
3. **阅读** — `lark_mail_message` 读单封邮件，`lark_mail_thread` 读整个会话
4. **回复** — `lark_mail_reply` / `lark_mail_reply_all`（默认存草稿，加 `confirm_send=true` 则立即发送）
5. **转发** — `lark_mail_forward`（默认存草稿，加 `confirm_send=true` 则立即发送）
6. **新邮件** — `lark_mail_send` 存草稿（默认），加 `confirm_send=true` 发送
7. **HTML body 预检（可选）** — 复杂 HTML body 提交前可先跑 `lark_mail_lint_html` 看 lint 会改 / 删什么；写信路径（`lark_mail_send` / `lark_mail_draft_create` / `lark_mail_reply` / `lark_mail_reply_all` / `lark_mail_forward` / `lark_mail_draft_edit` body op）已内置 autofix，普通正文不必先跑。详见 `lark_get_skill(domain="mail", section="html")` 中的「写入路径内置 HTML lint」章节
8. **确认投递** — 立即发送后用 `send_status` 查询投递状态，定时发送后在预定时间后再查询；取消定时发送用 `cancel_scheduled_send`
9. **编辑草稿** — `lark_mail_draft_edit` 修改已有草稿。正文编辑通过 `patch_file`：回复/转发草稿用 `set_reply_body` op 保留引用区，普通草稿用 `set_body` op
10. **已读回执** —
...
### CRITICAL — 首次使用任何工具前先查阅参数

无论是 Shortcut 工具（`lark_mail_triage`、`lark_mail_send` 等）还是原生 API，**首次调用前必须先通过 `lark_discover` 查看可用参数**，不要猜测参数名称：
...
`lark_discover` 输出即可用参数的权威来源。reference 文档中的参数表可辅助理解语义，但实际参数名称以 `lark_discover` 为准。
...
```
lark_invoke(tool_name="lark_mail_multi_entity_search", args={data: {"query": "<关键词>"}})
```
...
- **发送后必须调用 `send_status` 确认投递状态**；定时发送（`send_time`）在预定发送时间后再查询，取消定时发送用 `cancel_scheduled_send`（详见下方说明）

> **定时发送注意事项**：`send_time` 必须与 `confirm_send=true` 配合使用，不能单独使用。`send_time` 为 Unix 时间戳（秒），需至少为当前时间 + 5 分钟。
...
**立即发送（无 `send_time`）**：邮件发送成功后（收到 `message_id`），**必须**调用 `send_status` API 查询投递状态并向用户报告：
...
**定时发送（指定了 `send_time`）**：定时发送不会立即产生 `message_id`，`send_status` 在定时发送成功后会返回"待发送"状态，**不建议在定时发送后立即查询**。可在预定发送时间后再查询。如需取消定时发送：
...
**撤回操作：**
```
lark_invoke(tool_name="lark_mail_user_mailbox_sent_messages_recall", args={params: {"user_mailbox_id": "me", "message_id": "<message_id>"}})
```
...
**查询撤回进度：**
```
lark_invoke(tool_name="lark_mail_user_mailbox_sent_messages_get_recall_detail", args={params: {"user_mailbox_id": "me", "message_id": "<message_id>"}})
```
...
1. 分享单封邮件到群聊（默认 `receive_id_type=chat_id`）：
   ```
...
4. 如果不知道群聊 ID，先搜索：
   ```
   lark_im_chat_search(query="群名关键词")
   ```
...
**约束：**
- `event_*` 与 `send_time`（定时发送）互斥，不可同时使用
...
撰写邮件正文时，**默认使用 HTML 格式**（body 内容会被自动检测）。仅当用户明确要求纯文本时，才使用 `plain_text=true` 强制纯文本模式。

- HTML 支持粗体、列表、链接、段落等富文本排版，收件人阅读体验更好
- 所有发送类工具（`lark_mail_send`、`lark_mail_reply`、`lark_mail_reply_all`、`lark_mail_forward`、`lark_mail_draft_create`）都支持自动检测 HTML，可通过 `plain_text=true` 强制纯文本
...
- 写信时**必须**遵守邮件 HTML 写法规范 `lark_get_skill(domain="mail", section="html")` — **CRITICAL** 飞书邮箱已验证的最纯净美观写法集合
- `lark_mail_lint_html` 用法 `lark_get_skill(domain="mail", section="lint-html")` — 创建草稿前自检 / 修复 HTML 输出
- **官方模板库**（仓库 `assets/templates/` 目录）— 提供部分场景模板，可供参考；用 `lark_get_skill(domain="mail", section="assets/templates/<name>.html")` 按名取用某个模板

### 读取邮件：按需控制返回内容

`lark_mail_message`、`lark_mail_messages`、`lark_mail_thread` 默认返回 HTML 正文（`html=true`）。仅需确认操作结果（如验证标记已读、移动文件夹是否成功）时，用 `html=false` 跳过 HTML 正文，只返回纯文本，显著减少 token 消耗。
...
### 邮件模板（`lark_mail_template_create` / `lark_mail_template_update` / `template_id`）

模板的创建 / 更新由专用工具处理（自动做 Drive 上传 + `<img src>` 改写成 `cid:`）；发信类工具通过 `template_id="<id>"` 套用模板。

> **跟仓库 `assets/templates/` 的区别**：本节讲的是**飞书 OAPI 的个人邮件模板系统**（用户邮箱里的"我的模板"），可在飞书客户端管理；上面"仓库内置 HTML 模板库"是仓库里预制的飞书原生 HTML 文件，可供写信参考（用 `lark_get_skill(domain="mail", section="assets/templates/<name>.html")` 取用）。
...
- 列表 / 获取 / 删除 走原生 API：`lark_invoke(tool_name="lark_mail_user_mailbox_templates_list", ...)`。

**套用模板（5 个发信工具）**：`lark_mail_send` / `lark_mail_draft_create` / `lark_mail_reply` / `lark_mail_reply_all` / `lark_mail_forward` 均支持 `template_id="<id>"`。`template_id` 必须是**十进制整数字符串**。
...
没有 Shortcut 工具覆盖的操作才使用原生 API。调用步骤以本节为准（API Resources 章节的 resource/method 列表可辅助查阅）。
...
| Shortcut | 说明 |
|----------|------|
| `lark_mail_message` | Use when reading full content for a single email by message ID. Returns normalized body content plus attachments metadata, including inline images. 详见 `lark_get_skill(domain="mail", section="message")`。 |
| `lark_mail_messages` | Use when reading full content for multiple emails by message ID. Prefer this shortcut over calling raw mail user_mailbox.messages batch_get directly, because it base64url-decodes body fields and returns normalized per-message output that is easier to consume. 详见 `lark_get_skill(domain="mail", section="messages")`。 |
...
```
lark_discover(query="mail.<resource>.<method>")   # 调用 API 前必须先查看参数结构
lark_invoke(tool_name="lark_mail_<resource>_<method>", args={...}) # 调用 API
```
