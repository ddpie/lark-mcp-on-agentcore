...
```
# 读取一封邮件（默认包含 HTML 正文）
lark_mail_message(message_id="<message-id>")

# 仅纯文本正文（更小的负载，适合 AI 处理）
lark_mail_message(message_id="<message-id>", html=false)

# 指定邮箱
lark_mail_message(mailbox="user@example.com", message_id="<message-id>")

# JSON 输出（脚本友好）
lark_mail_message(message_id="<message-id>", format="json")
```
...
# 3. 发送回复
lark_mail_reply(message_id="<id>", body="...")
```
...
lark_mail_message(message_id="<id>", format="json")

# 2. 仅为需要的 ID 获取下载 URL
lark_discover(query="mail.user_mailbox.message.attachments.download_url")
lark_invoke(tool_name="lark_mail_user_mailbox_message_attachments_download_url", args={params: {"user_mailbox_id": "me", "message_id": "<id>", "attachment_ids": ["att_xxx", "att_yyy"]}})
```
...
- `lark_invoke(tool_name="lark_mail_user_mailbox_messages_list", ...)` — 列出收件箱邮件（获取 `message_id`）
