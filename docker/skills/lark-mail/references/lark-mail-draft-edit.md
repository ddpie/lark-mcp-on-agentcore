...
编辑已有的邮件草稿。工具会读取当前原始 EML，应用最小化补丁，然后将更新后的草稿写回。
...
**判断方法：** 运行 `inspect=true`，若返回 `has_quoted_content: true`，说明草稿包含引用区（由 `lark_mail_reply` 或 `lark_mail_forward` 生成）。
...
lark_mail_draft_edit(draft_id="<draft-id>", inspect=true)

# 打印补丁模板
lark_mail_draft_edit(print_patch_template=true)
```
...
3. 生成符合该结构的补丁文件
4. 运行 `patch_file="./patch.json"`

`patch_file` 接受项目专用的类型化补丁 JSON 格式，不是 RFC 6902 JSON Patch。
...
**如何获取定位字段：** 不同类型附件有不同的定位字段，都从 `inspect=true` 获取：
...
插入签名到正文末尾（引用块之前）。如已有签名则先移除再插入。运行 `lark_mail_signature` 获取可用签名 ID。签名中的模板变量会自动替换，内联图片自动下载嵌入。
...
- 通过 `inspect=true` 返回的 `has_quoted_content` 字段可判断草稿是否包含引用区
- 通过 `inspect=true` 返回的 `has_signature` / `signature_id` 字段可判断草稿是否包含签名
...
`remove_attachment` 统一处理普通附件和超大附件；根据 `inspect=true` 输出选择对应的定位字段。
...
- `lark_mail_draft_create` — 创建新草稿
- `lark_invoke(tool_name="lark_mail_user_mailbox_drafts_get", ...)` — 获取草稿原始内容
