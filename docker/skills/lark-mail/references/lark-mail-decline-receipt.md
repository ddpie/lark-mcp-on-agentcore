...
关闭收到邮件的已读回执请求 banner，**但不向发件人发送回执**。**本工具仅在对方邮件请求了已读回执（`READ_RECEIPT_REQUEST` 标签，系统 ID `-607`）时使用**。对齐飞书客户端上已读回执 banner 右侧的"不发送"按钮。

本工具对应 MCP tool：`lark_mail_decline_receipt`。

## 使用时机

决策分支：拉信看到 `READ_RECEIPT_REQUEST` 标签 → **必须先问用户**：

- 用户愿意告知对方"已读" → `lark_mail_send_receipt`
- 用户不愿意告知但想消掉提示 → `lark_mail_decline_receipt`（本工具）
...
```
# 标准用法
lark_mail_decline_receipt(message_id="<message-id>")

# 指定邮箱（公共邮箱场景）
lark_mail_decline_receipt(mailbox="shared@example.com", message_id="<message-id>")
```
...
> 注意本工具没有 confirm 参数 —— 它只是移除一个本地 label，不对外发信，Risk 级别是 `write` 而非 `high-risk-write`。
...
# → label_ids 包含 ["UNREAD", "READ_RECEIPT_REQUEST"]
...
# 3. 用户选了"不发送" → 
lark_mail_decline_receipt(message_id="msg-1")
```
...
- ❌ 拿 `lark_mail_decline_receipt` 当"标记已读"——它只移 `READ_RECEIPT_REQUEST` 一个标签，不改 `UNREAD`
- ❌ 在没有 `READ_RECEIPT_REQUEST` 标签的邮件上调用 —— 虽然幂等返回 `already_cleared`，但多发一次 GET 无意义

## 相关工具

- `lark_mail_send_receipt` — 同意回执（发一封系统样式的已读回执邮件）
