...
`lark_mail_lint_html` 是邮件 HTML 正文的本地预检工具（read-only，无网络 IO）。

- 校验 HTML 是否符合飞书邮箱的兼容性 / 安全 / 原生写法要求；
- 自动修复非法或不规范写法（autofix 始终启用），输出 `cleaned_html`；
- 不写入任何邮箱状态，不调用任何 OAPI。

写信链路（`lark_mail_send` / `lark_mail_draft_create` / `lark_mail_reply` / `lark_mail_reply_all` / `lark_mail_forward` / `lark_mail_draft_edit` body op）已**强制内置**同一份 lint，提交前会自动净化 HTML。默认返回不携带任何 lint 字段以保持响应小巧；加 `show_lint_details=true` 可拿到完整 `lint_applied[]` / `original_blocked[]` 两个 Finding 数组（不再返回任何 `*_count` 字段，调用方需要 count 时 `len(arr)` 即可，详见 `lark_get_skill(domain="mail", section="html")` 中的「写信 shortcut 的 lint 返回值」章节）。本工具是写信链路 lint 的预览版，行为一致，调用更轻量，适合：
...
```
# 直接传 HTML
lark_mail_lint_html(body="<p>正文</p>")

# 从文件读 HTML（路径必须在 cwd 子树内）
lark_mail_lint_html(body_file="./template.html")

# 查看完整 lint 详情
lark_mail_lint_html(body_file="./template.html", show_lint_details=true)
```
...
| `show_lint_details` | 否 | 默认 `false`。`true` 时同时返回 `warnings[]` / `errors[]` 完整 Finding 数组；默认仅返回 `cleaned_html`，避免复杂模板触发数十条装饰性 warning 把响应撑大几千 token |
| `format` | 否 | `json`（默认）/ `pretty` / `table` / `csv` / `ndjson` |

## 返回值

**默认返回**（仅 `cleaned_html`，token-frugal）：
...
| `warnings[]` | 警告级 finding 数组（**仅 `show_lint_details=true` 时返回**）。无违规时输出 `[]` |
| `errors[]` | 错误级 finding 数组（**仅 `show_lint_details=true` 时返回**）。无违规时输出 `[]` |
...
下面是用 `lark_mail_lint_html(body="<INPUT>", show_lint_details=true)` 实跑得到的典型 case（加 `show_lint_details=true` 才能看到 finding；默认只返回 `cleaned_html`），覆盖 error 类（强制删）和 warning 类（自动修复）。
...
- 写信 shortcut（已内置同一份 lint）：`lark_mail_send` / `lark_mail_draft_create` / `lark_mail_reply` / `lark_mail_reply_all` / `lark_mail_forward` / `lark_mail_draft_edit`
- 知识文档：`lark_get_skill(domain="mail", section="html")`
