...
**核心规则**：不管图片从哪来（本地文件、URL、文档中的 `docx_image` token、其他域的 Drive token），都必须通过 `lark_docs_media_upload(parent_type="whiteboard", parent_node="<目标画板token>")` 上传，拿到画板专属的 media token 后才能在 DSL 中使用。直接使用非 `whiteboard` 域的 token 会导致画板 API 报 500（错误码 2891001）或图片在文档中消失。
...
| 文档中的图片 token | `lark_docs_media_download(token="<token>", output="./photo.png")` |
| 其他域的 Drive token | `lark_docs_media_download(token="<token>", output="./photo.png")` |
...
**必须**使用 `lark_docs_media_upload` 并指定 `parent_type="whiteboard"` 上传：

```
lark_docs_media_upload(file="./photo1.jpg", parent_type="whiteboard", parent_node="<whiteboard_token>")
...
```
lark_docs_media_upload(file="./photo1.jpg", parent_type="whiteboard", parent_node="<whiteboard_token>")  # → <media_token_1>
lark_docs_media_upload(file="./photo2.jpg", parent_type="whiteboard", parent_node="<whiteboard_token>")  # → <media_token_2>
lark_docs_media_upload(file="./photo3.jpg", parent_type="whiteboard", parent_node="<whiteboard_token>")  # → <media_token_3>
```
...
| 错误现象 | 原因 | 解决 |
|---------|------|------|
| 画板 API 返回 500（2891001） | 使用了非 `whiteboard` 域 token（如 `docx_image`、Drive file token） | 下载图片后用 `lark_docs_media_upload(parent_type="whiteboard")` 重新上传 |
