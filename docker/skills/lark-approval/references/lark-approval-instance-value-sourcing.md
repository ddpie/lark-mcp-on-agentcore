...
本文用于回答一个固定问题：在调用 `lark_approval_instances_create` 发起原生审批实例时，**每个要填写的值从哪里拿**。
...
- 审批定义、`approval_code`、`is_external`、`create_link` 等基础信息，默认从 `approvals search` 获取。
- 控件 `id`、`type`、选项值、子控件结构，默认从 `approvals get` 返回的 `form` 获取。
- 节点 key、`need_approver`、`approver_chosen_multi` 等节点信息，默认从 `approvals get` 返回的 `node_list` 获取。
...
- 如果用户给的是姓名、邮箱或账号，先用 `lark_get_skill(domain="contact")` 解析成 `open_id`。

### 部门 `department`

- 最优先：用户直接提供 `open_department_id`。
- 若用户说“我的部门”或“张三的部门”，先用 `lark_get_skill(domain="contact")` 查询对应人员信息，再取其所属部门里的 `open_department_id`。
- 如果查到该人员只有一个部门，可直接使用。
...
以下控件组的结构仍按 `lark_get_skill(domain="approval", section="instance-form-control-parameters")` 组装：
...
- 控件组自身和子控件的 `id` / `type` 从 `approvals get` 返回的 `form` 中识别。
- 组内单选/多选或业务枚举值，优先从 `approvals get` 返回的 `form` 的选项结构中取。
...
| 场景 | 处理 |
|---|---|
| 用户说“找张三当审批人” | 用 `lark_get_skill(domain="contact")` 解析张三，取 `open_id` |
