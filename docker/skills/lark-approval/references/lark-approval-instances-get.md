...
# 表格格式输出，便于快速浏览顶层字段
lark_invoke(tool_name="lark_approval_instances_get", args={params: {"instance_code":"<INSTANCE_CODE>"}, format: "table"})
```
...
- **这是最适合做“详情确认”的只读调用**：当你已经拿到 `instance_code`，需要确认表单、当前节点、任务状态、审批动态时，优先使用它。
...
```
# 同意审批任务（高风险写操作，需 _confirm=true）
lark_invoke(tool_name="lark_approval_tasks_approve", args={data: {"instance_code":"<INSTANCE_CODE>","task_id":"<TASK_ID>"}, _confirm: true})
...
# 催办审批任务（高风险写操作，需 _confirm=true）
lark_invoke(tool_name="lark_approval_tasks_remind", args={data: {"instance_code":"<INSTANCE_CODE>","task_ids":["<TASK_ID>"]}, _confirm: true})
```
