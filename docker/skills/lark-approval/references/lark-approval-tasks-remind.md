...
> [!CAUTION]
> 这是**高风险写操作**。真正执行时需要传 `_confirm=true`；只有用户已明确要催办该审批且目标实例、目标任务都无误时才执行。不要在未获用户明确同意时静默追加 `_confirm=true`。
...
```
# 催办单个审批任务
lark_invoke(tool_name="lark_approval_tasks_remind", args={data: {"instance_code":"<INSTANCE_CODE>","task_ids":["<TASK_ID>"],"comment":"请尽快审批该单据"}, _confirm: true})

# 同一实例下催办多个任务
lark_invoke(tool_name="lark_approval_tasks_remind", args={data: {"instance_code":"<INSTANCE_CODE>","task_ids":["<TASK_ID_1>","<TASK_ID_2>"],"comment":"请相关审批人尽快处理"}, _confirm: true})
```
...
- **执行前先向用户确认**：尤其在一次催办多个任务、任务来源不明确或需让用户复核催办对象时，先让用户核对再执行。
