...
# 使用 page_token 翻页
lark_invoke(tool_name="lark_approval_approvals_search", args={data: {"keyword":"请假", "page_token":"example_page_token"}})

# 表格格式输出，便于快速浏览候选定义
lark_invoke(tool_name="lark_approval_approvals_search", args={data: {"keyword":"出差"}, format: "table"})
```
...
- **`is_external=true` 时不要调用 `lark_approval_instances_create`。** 这类定义属于三方审批，优先返回 `create_link` 并说明需要通过链接发起。
- **只有 `is_external=false` 的原生定义，才继续 `approvals get`。**
- **如果用户已经明确给出 `approval_code`，不要再 search。** 直接执行 `approvals get`。
