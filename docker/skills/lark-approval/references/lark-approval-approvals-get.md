...
lark_invoke(tool_name="lark_approval_approvals_get", args={params: {"approval_code":"<APPROVAL_CODE>"}})

# 表格格式输出，便于快速浏览顶层字段
lark_invoke(tool_name="lark_approval_approvals_get", args={params: {"approval_code":"<APPROVAL_CODE>"}, format: "table"})
```
...
| `approval_code` | 是 | 审批定义 Code；通常来自 `approvals search` 的结果 |
...
`form` 最重要的作用是帮助 agent **识别怎么组装 `instances create` 请求体里的 `form`**，而不是直接把它原样提交出去。
...
**注意：`approvals get` 返回的 `form` 不是 `instances create` 可直接复用的 payload 模板。** 它是“定义快照”，主要用于识别字段结构与选项值范围。
...
- **如果用户已经明确给了 `approval_code`，直接用这个调用。** 不必再走 `approvals search`。
...
```
# 发起原生审批实例（高风险写操作，需 _confirm=true）
lark_invoke(tool_name="lark_approval_instances_create", args={data: {"approval_code":"<APPROVAL_CODE>","form":"[...]"}, _confirm: true})
```

如果需要进一步理解控件取值与节点参数，优先参考：

- `lark_get_skill(domain="approval", section="instance-form-control-parameters")`
- `lark_get_skill(domain="approval", section="instance-value-sourcing")`
- `lark_get_skill(domain="approval", section="initiate")`
