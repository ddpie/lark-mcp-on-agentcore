...
## Resource auto-download (`download_resources`, opt-in)

`lark_im_chat_messages_list`, `lark_im_messages_mget`, and `lark_im_threads_messages_list` accept an **opt-in** `download_resources=true` flag. It is **off by default** — when omitted, output and the request count are identical to before (no `resources` block, no extra round-trips).
