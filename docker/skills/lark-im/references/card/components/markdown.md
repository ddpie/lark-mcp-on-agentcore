...
| 带图标链接 | `<link icon='chat_outlined' …>文案</link>`（icon token 见 `lark_get_skill(domain="im", section="card/resource/icons")`） |
| 彩色文本 | `<font color='red'>红字</font>`（color 枚举见 `lark_get_skill(domain="im", section="card/resource/colors")`；链接文本不可着色） |
...
- **慎用大标题**：`#` / `##` / `###` 一~三级标题字号过大、显丑，正文里一律用 `**加粗**` 替代来突出重点。**唯一例外**是「指标卡」里用 `##` 放大数值（见 `lark_get_skill(domain="im", section="card/lark-im-card-style")` 视觉规范）。
- **少用 `markdown` 的 `margin`**：间距优先交给父容器的 `vertical_spacing` / `padding`，多数情况置 `0px`；仅精细缩进时设非零值（见 `lark_get_skill(domain="im", section="card/lark-im-card-style")` 间距纪律）。
