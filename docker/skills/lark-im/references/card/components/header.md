...
**标签 color 枚举**：`neutral`/`blue`/`turquoise`/`lime`/`orange`/`violet`/`indigo`/`wathet`/`green`/`yellow`/`red`/`purple`/`carmine`。深浅档位及 RGBA 见 `lark_get_skill(domain="im", section="card/resource/colors")`。

## 选色建议

按场景选 template 颜色见 `lark_get_skill(domain="im", section="card/lark-im-card-style")` 意图表。常见语义：green=成功/完成，orange=警告，red=错误/危险，grey=失效/归档，blue=通用信息。
