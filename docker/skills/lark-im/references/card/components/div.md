...
| `text.tag` | 是 | String | plain_text | `plain_text` 或 `lark_md`（部分 Markdown，语法见 `lark_get_skill(domain="im", section="card/components/markdown")`） |
...
| `text.text_color` | 否 | String | default | 颜色枚举（见 `lark_get_skill(domain="im", section="card/resource/colors")`），仅 `plain_text` 生效 |
| `text.text_align` | 否 | String | left | `left` / `center` / `right` |
| `text.lines` | 否 | Int | / | 最大显示行数，超出 `...` 省略 |
| `icon` | 否 | Object | / | 前缀图标，见下 |
| `icon.tag` | 否 | String | / | `standard_icon`（用 `token`+`color`，token 见 `lark_get_skill(domain="im", section="card/resource/icons")`）或 `custom_icon`（用 `img_key`） |
