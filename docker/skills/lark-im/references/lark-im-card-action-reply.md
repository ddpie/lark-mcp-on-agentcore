...
> ⚠️ **Bot-only — not available via the MCP server.** This entire workflow requires bot
> identity (`tenant_access_token`): the callback stream is delivered through real-time event
> consumption (which is not exposed by the Remote MCP server), and the delayed-update API
> (`/open-apis/interactive/v1/card/update`) is bot-only. The MCP server is **user-identity
> only**, so card callbacks cannot be received or answered through it. This page is retained
> for reference; the operations below are not user-callable.
...
When a `card.action.trigger` event arrives (**each event line is one event — process it immediately**):
...
⚠️ The delayed-update API is **bot-only** (`tenant_access_token`) and not available via the MCP
server. For reference, the underlying raw API is `POST /open-apis/interactive/v1/card/update`
with a `data` body of `{"token":"<token>","card":<new_card_json>}`.
...
- **`action_value` type is developer-defined**: the original may be an object or a plain string.
  Parse defensively (swallow JSON parse errors) or check the type before parsing.
