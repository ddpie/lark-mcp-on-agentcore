...
- Use `markdown` when you want basic Markdown-style rendering and you accept that the tool will normalize and rewrite parts of the content before sending.
...
- The tool normalizes common variants like `<at id=...>` and `<at open_id=...>` into `user_id`, but you should still document examples with `user_id`
