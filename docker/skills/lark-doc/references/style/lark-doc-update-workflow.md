...
2. **Execute（执行）** — 调用相应的 MCP tool，或 **spawn** Agent 子任务并行推进
...
1. **选择读取范围**（节省上下文的关键）：
   - 用户只改某一节 / 文档较大 → 先 `lark_docs_fetch(api_version="v2", scope="outline", max_depth="2")` 拿目录，再 `lark_docs_fetch(api_version="v2", scope="section", start_block_id="<目标标题id>", detail="with-ids")` 精读该节（`section` 会自动展开到下一个同级/更高级标题前，不用手动算结束 block id）
   - 需要精确跨节区间 → `lark_docs_fetch(api_version="v2", scope="range", start_block_id="xxx", end_block_id="yyy")`（或 `end_block_id="-1"` 读到末尾）
   - 用户只给了模糊关键词 → `lark_docs_fetch(api_version="v2", scope="keyword", keyword="xxx", context_before="1", context_after="1", detail="with-ids")`
...
SVG SubAgent 必须收到：文档 token、插入位置（标题/block ID）、图表目标、源内容片段、`lark_get_skill(domain="doc", section="xml")` 引用，以及 `lark_get_skill(domain="doc", section="whiteboard")` 中的 "SVG 设计 Workflow" 指南。它只负责插入一个 `<whiteboard type="svg">...</whiteboard>`，不改其他正文，也不读取 `lark-whiteboard`。

已有画板更新 SubAgent 必须收到：board_token、图表目标、推荐画板类型、源内容片段、`lark_get_skill(domain="whiteboard")` 引用。它只负责写入画板，不改文档正文。

**上下文节省提示**：Agent 如需在自己负责的章节内重新读取内容，优先用 `lark_docs_fetch(api_version="v2", scope="section", start_block_id="<章节标题id>")`（自动覆盖整节），或 `lark_docs_fetch(scope="range", start_block_id="xxx", end_block_id="yyy")` 精确区间，只拉自己的章节，不要重复拉全文。
