...
**上下文节省提示**：Agent 如需在自己负责的章节内重新读取内容，优先用 `lark_docs_fetch(scope="section", start_block_id="<章节标题id>")`（自动覆盖整节），或 `lark_docs_fetch(scope="range", start_block_id="xxx", end_block_id="yyy")` 精确区间，只拉自己的章节，不要重复拉全文。
