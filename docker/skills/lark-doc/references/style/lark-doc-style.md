...
本文只讲"何时用、什么风格"；具体标签 / 命令语法见 `lark_get_skill(domain="doc", section="xml")`。
...
- **画板**：默认用文字，只在**图示明显比文字更易懂**（流程、架构、时间线、对比、占比等）或用户要求时才用。怎么插、用哪种类型见 `lark_get_skill(domain="doc", section="xml")` 与 `lark_get_skill(domain="doc", section="whiteboard")`。
- **颜色**：默认朴素、不上色；需要时保持语义一致，按下表选择对应颜色，不为装饰上色。可用色见 `lark_get_skill(domain="doc", section="xml")` 的「美化系统」。
