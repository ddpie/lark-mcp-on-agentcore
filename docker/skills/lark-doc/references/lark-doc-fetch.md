...
```
# 获取文档（默认 XML，simple）
lark_docs_fetch(api_version="v2", doc="https://xxx.feishu.cn/docx/Z1Fj...tnAc")

# Markdown 格式
lark_docs_fetch(api_version="v2", doc="Z1Fj...tnAc", doc_format="markdown")

# 带 block ID（用于后续 block 级更新）
lark_docs_fetch(api_version="v2", doc="Z1Fj...tnAc", detail="with-ids")

# 只拿目录
lark_docs_fetch(api_version="v2", doc="Z1Fj...tnAc", scope="outline", max_depth="3")

# 按 block id 区间精读
lark_docs_fetch(api_version="v2", doc="Z1Fj...tnAc", scope="range", start_block_id="blkA", end_block_id="blkB", detail="with-ids")

# 读整个章节（以标题 id 为锚点，自动展开到下一个同级/更高级标题前）
lark_docs_fetch(api_version="v2", doc="Z1Fj...tnAc", scope="section", start_block_id="<标题id>", detail="with-ids")

# 按关键词定位（多关键词用 | 分隔，任一命中即返回）
lark_docs_fetch(api_version="v2", doc="Z1Fj...tnAc", scope="keyword", keyword="部署|发布|上线")
```
...
`content` 的格式由 `doc_format` 决定；`im-markdown` 仅用于获取内容后在 `lark-im` 场景下使用。设置 `scope` 时会被 `<fragment>` 包裹，详见上文"局部读取的输出结构"。
