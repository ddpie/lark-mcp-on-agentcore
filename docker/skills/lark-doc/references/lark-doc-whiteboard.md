...
| 已有画板需要更新内容              | 先 `lark_docs_fetch(api_version="v2")` 获取 `board_token`，跳至步骤 3B |
...
主 Agent 启动 SubAgent，让它用 `lark_docs_create(api_version="v2")` / `lark_docs_update(api_version="v2")` 插入：
...
`<radialGradient>` / `<filter>` / `<clipPath>` 等装饰特性画板完全不支持，会导致渲染问题（见下方）
