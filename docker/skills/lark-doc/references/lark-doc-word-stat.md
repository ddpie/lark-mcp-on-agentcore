...
先用 XML full 内容获取文档，再把 `lark_docs_fetch(format="json")` 返回的完整 JSON envelope 作为 `stdin` 交给脚本（脚本用 `--lark-json` 解析 `data.document.content`）：

```
lark_docs_fetch(doc="<URL>", doc_format="xml", detail="full", format="json")
lark_exec_script(script="lark-doc/scripts/doc_word_stat.py", args=["--protocol", "xml", "--lark-json", "--pretty"], stdin="<上一步 lark_docs_fetch 返回的完整 JSON>")
```

`<URL>` 可以是用户给出的 docx/wiki URL，也可以是可被 `lark_docs_fetch` 解析的 token。

如需在自动化或回归验证中发现未覆盖块类型，追加严格参数：

```
lark_exec_script(script="lark-doc/scripts/doc_word_stat.py", args=["--protocol", "xml", "--lark-json", "--pretty", "--fail-on-unsupported", "--fail-on-unknown"], stdin="<lark_docs_fetch 返回的完整 JSON>")
```
