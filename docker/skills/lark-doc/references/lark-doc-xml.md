...
- `<whiteboard>` — 简单图由 SubAgent 直接插入 `<whiteboard type="svg">完整自包含 SVG</whiteboard>`；复杂图使用 `<whiteboard type="blank"></whiteboard>` 先创建空白画板，再按 `lark_get_skill(domain="doc", section="whiteboard")` 启动 SubAgent 调用 `lark-whiteboard` 写入；
...
- 常用 emoji： bulb checkmark crossmark warning memo question exclamation thumbsup heart pin flag star
