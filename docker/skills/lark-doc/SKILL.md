...
> **API 版本：本 skill 使用 v2 API。所有 `lark_docs_create`、`lark_docs_fetch`、`lark_docs_update` 调用必须携带 `api_version="v2"`。**
...
| `lark_docs_fetch` | Fetch Lark document content (XML / Markdown) |
...
| `lark_whiteboard_update` | Alias of `lark_whiteboard_update`. Update an existing whiteboard with DSL, Mermaid or PlantUML. Refer to lark-whiteboard skill for details. |
