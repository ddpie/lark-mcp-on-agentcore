...
> **边界**：本文只讲"公式怎么写对"；公式**怎么写入表格**（`lark_sheets_cells_set` / 模板单元格 + `copy_to_range` / 容错回读）见 `lark_get_skill(domain="sheets", section="write-cells")` 与 `lark_get_skill(domain="sheets", section="core-operations")`。本文不含 shortcut，铁律见 `lark_get_skill(domain="sheets", section="core-operations")`。
...
- **公式字符串用飞书范围语法**：写 `H:H`、`A2:B5`，**禁止** `H2:H` / `2:2`。这与工具参数（如 `range`）的 A1 表示法（`A1:D3`、`1:1`）写法不同，两者混淆会导致调用失败或公式报错。
...
> 本段是"飞书不支持函数"的**唯一权威清单**（`lark_get_skill(domain="sheets", section="core-operations")` 不再单列，统一指向这里）。以下函数在飞书里不存在或被禁用，禁止主动使用；用户明确要求时应拒绝并提供替代方案：
