...
> **边界**：本文只讲"公式怎么写对"；公式**怎么写入表格**（`lark_sheets_cells_set` / 模板单元格 + `copy_to_range` / 容错回读）见 `lark_get_skill(domain="sheets", section="write-cells")`。**公式写入完成后的强制收尾**见 `lark_get_skill(domain="sheets", section="formula-verify")`：不要把"翻译对了"误当成"已经交付完成"。本文不含 shortcut，通用编辑准则见主 SKILL.md「飞书表格编辑准则」。
...
2. 公式一旦落表，就默认进入 `lark_get_skill(domain="sheets", section="formula-verify")` 的收尾阶段。
3. 最终必须跑 `lark_sheets_formula_verify` 收敛到 `status='success'`；`errors_found` / `partial` 都不算完成。
