...
5. `border_styles`（四边框）
6. **`merged_cells`（合并范围）**——续写场景必查（高频致命错误）：用 `lark_sheets_sheet_info(include="merges")` 读原数据区域的合并信息。**原行有跨列合并**（如标题行 `A1:G1` 合并）时，新行**必须**用 `lark_sheets_cells_{merge|unmerge}` 工具复制相同合并模式到新行（如续写第 3 个周报块的标题行 `A23:G23` 必须合并）。仅传 cells 数组的 5 类样式不够——合并范围要单独靠 `lark_sheets_cells_{merge|unmerge}` 工具落地（典型反例：续写多周记录表时，新增周次的标题行未合并，视觉上与原前几周风格不一致）
...
> - **浮动图片**：图片悬浮在单元格上方，可自由定位和调整大小，不属于单元格内容。→ 使用 `lark_get_skill(domain="sheets", section="float-image")`。
...
💡 **写入公式前先按迁移规则改写**：如果公式来自 Excel 或包含数组场景，先读取并遵循 `lark_get_skill(domain="sheets", section="formula-translation")` 的规则完成改写，再把最终公式写入 `formula` 字段。
...
4. **去重 / 筛选函数**：飞书**支持** `UNIQUE` / `FILTER` / `SPLIT`（原生数组函数，详见 `lark_get_skill(domain="sheets", section="formula-translation")`），可直接用；`DISTINCT` 不是飞书函数，去重用 `UNIQUE`。大数据量去重 / 分组也可用透视表（`lark_sheets_pivot_{create|update|delete}`，值字段聚合方式选 count）
...
⚠️ **收到 `formula_errors` 反馈后不要只打补丁（高频致命错误）**：`lark_sheets_cells_set` 返回值里若出现 `formula_errors: [{cell, formula, error_type, detail}]`，说明某些 cell 公式编译失败（`error_type=compile_failed` 通常是函数语法错如 `SPLIT(x)[1]` 的下标取值飞书不支持（SPLIT 本身支持，取第 N 项用 `INDEX(SPLIT(...),N)`）；`non_formula` 是 `=` 开头但解析不通过）。此时**禁止只聚焦修报错点的局部语法**（如仅把 `[1]` 换成 `INDEX(..,1)`），必须：
...
满足上述时，**不要在本 skill 里猜样式**，直接去读 `lark_get_skill(domain="sheets", section="visual-standards")` 的「场景一 → 1A. 添加汇总行 / 表头行」章节，按那里的样式要点配齐 `font.bold / horizontal_alignment / background_color / border_styles`。
...
带显示文本的超链接、@人、@文档这类富内容**必须**走 `lark_sheets_cells_set` 的 `rich_text` 字段（`cells[].rich_text` 数组，每段一个对象、带 `type`），**不能**直接传普通字符串——纯字符串只会被当作纯文本存进单元格。完整字段用 `lark_discover(query="sheets.cells-set")` 查看，常用段类型：
...
`mention_type`（类型编号）等可选字段以 `print_schema` 输出为准。
...
两个 flag **必须传一个、且只能传一个**——同时传或都不传，CLI 会立刻报错。`source_range` 用 A1 + sheet 前缀写法（如 `'Sheet1'!T1:T3`，sheet 名按 A1 标准单引号包裹），可以指同 sheet 也可以指其它 sheet（如 `'Refs'!A1:A10`）。
...
`colors` 长度**可以短于**选项数（list 模式短于 `options` 长度，listFromRange 模式短于 `source_range` 的单元格数），未指定的选项按内置色板循环补色；但**不能长于**——CLI 在 Validate 阶段就会拦截，错误形如 `colors length (4) must not exceed dropdown source size (3)`。
...
```
lark_sheets_dropdown_set(url="https://...", sheet_id="<id>", range="A2:A100", options=["待开始","进行中","已完成","已取消"])
```

**`options` 模式 — 指定颜色**（4 个选项配 3 个颜色，第 4 个按色板补）：

```
lark_sheets_dropdown_set(url="https://...", sheet_id="<id>", range="A2:A100", options=["待开始","进行中","已完成","已取消"], colors=["#bff7d9","#FFE699","#bacefd"])
```

**`source_range` 模式**（先在 `'Sheet1'!T1:T3` 维护「男/女/保密」三行，再让 `B2:B21` 引用它）：

```
lark_sheets_dropdown_set(url="https://...", sheet_id="<id>", range="B2:B21", source_range=true, colors=["#cce8ff","#ffd6e7","#e6e6e6"])
```

**纯白下拉**（明确告诉用户"不要彩色"时才用）：

```
lark_sheets_dropdown_set(url="https://...", sheet_id="<id>", range="A2:A100", options=["低","中","高"], highlight="false")
```
...
>
> ⚠️ **sheet 前缀里的表名一律「裸写」，不要加引号**——这条对所有带 sheet 前缀的 range 入参通用（`source_range`、`lark_sheets_cells_batch_set_style` / `lark_sheets_cells_batch_clear` / `lark_sheets_dropdown_update` 的 `ranges` 等）。即使表名含点或空格（如 `2025.9`、`一月份 `），也直接写 `2025.9!A1`；**不要**按电子表格习惯写成 `'2025.9'!A1`——引号会被当成表名的一部分，导致 `sheet "'2025.9'" not found`。

`lark_sheets_dropdown_update`（多 range 批量更新）的所有 flag 语义与 `lark_sheets_dropdown_set` 完全一致；只是目标 `ranges` 由单值变成 JSON 数组（每项带 sheet 前缀），同一份选项 + 配色应用到所有 range。
...
| 写入含公式、样式、批注、图片、数据校验等任意富写入 | `lark_sheets_cells_set` | 唯一支持完整字段的 shortcut |
| 只改已有 cell 的样式，不动 value/formula | `lark_sheets_cells_set_style` | 拍平 10 个样式字段为独立 flag；不触发不必要的值写入 |
| 单 cell 嵌入图片 | `lark_sheets_cells_set_image` | 比 `lark_sheets_cells_set` 参数更简短 |
| 大量纯值 + 需要表头样式/边框 | 先用 `lark_sheets_csv_put` 写值，再用 `lark_sheets_cells_set_style` 补样式 | 分工配合，入参最短 |

**优先级**：常规纯值写入优先 `lark_sheets_csv_put`（最短入参，直接传 CSV 文本）；含公式/样式/批注/图片才用 `lark_sheets_cells_set`。

⚠️ `lark_sheets_csv_put` 只写纯值，**不会**携带公式/样式/批注/图片；公式字符串以 `=` 开头会被当作字面量文本落地。如果数据里需要公式或样式，**必须**用 `lark_sheets_cells_set`（或"写值 + 补样式"两步法）。
...
| `cells` | string + File + Stdin（复合 JSON） | required | JSON：2D 数组 `[[{cell},...],...]`，维度与 `range` 完全一致；每个 cell 可含 `value` / `formula` / `cell_styles` / `note` / `rich_text`（含 `type="embed-image"` 单元格嵌图）等，完整字段跑 `print_schema` |
| `allow_overwrite` | bool | optional | 允许覆盖非空 cell（默认 true）；设为 false 时遇非空 cell 报错 |
| `max_cells` | int | optional | 防爆，默认 50000（隐藏 flag：不在 `help` 列出，但可正常传入） |
...
| `border_styles` | string + File + Stdin（复合 JSON） | optional | 边框配置 JSON：`{ top: {style,color,weight}, bottom: ..., left: ..., right: ... }`；4 方向结构相同 |
...
| `range` | string | optional | `start_cell` 的别名（与 `lark_sheets_csv_get` / `lark_sheets_cells_set` 一致，用 `range` 定位）；传区间（如 A1:H17）时自动取其左上角单元格（隐藏 flag：不在 `help` 列出，但可正常传入） |
...
"工具选择"段已讲清纯值（`lark_sheets_csv_put`）vs 富写入（`lark_sheets_cells_set`）。下表补 CLI 侧的 `lark_sheets_cells_set` **兄弟拆分**，以及不属于本 skill 的**跨 skill 转介绍**——避免 agent 用 `lark_sheets_cells_set` 硬扛所有写入场景。
...
| **插行/列 + 写入** 这种多步组合，且要原子 | `lark_sheets_batch_update`（跨 skill） | 多次独立 `lark_sheets_cells_set`（非原子；插入会扰动后续 range） |
| 在**多个不连续 range** 上应用同一组样式 | `lark_sheets_cells_batch_set_style`（跨 skill） | 多次 `lark_sheets_cells_set_style`（非原子） |
...
lark_sheets_cells_set(url="https://example.feishu.cn/sheets/shtXXX", sheet_name="Sheet1", range="A1:B2", cells=[[{"value":"name"},{"value":"score"}],[{"value":"alice"},{"value":95}]])

# 富 cell（公式 + 样式，cells 是二维矩阵每元素一个 cell schema）
lark_sheets_cells_set(spreadsheet_token="shtXXX", sheet_id="$SID", range="C2:C10", cells="@rich-cells.json")
```
...
只改样式，不动 value / formula。10 个 cell_styles 字段拍平为独立 flag，边框走 `border_styles` JSON。

```
# 加粗 + 黄底
lark_sheets_cells_set_style(url="...", sheet_name="Sheet1", range="A1:B2", font_weight="bold", background_color="#FFFF00")

# 配套边框
lark_sheets_cells_set_style(url="...", sheet_id="$SID", range="A1:D10", font_size="12", horizontal_alignment="center", border_styles={"top":{"style":"solid","color":"#000","weight":"thin"},"bottom":{"style":"solid","color":"#000","weight":"thin"}})
```
...
```
# 内联 CSV
lark_sheets_csv_put(url="https://example.feishu.cn/sheets/shtXXX", sheet_name="Sheet1", start_cell="A1", csv="$'name,score\nalice,95\nbob,87'")

# 从文件
lark_sheets_csv_put(spreadsheet_token="shtXXX", sheet_id="$SID", start_cell="A1", csv="@data.csv")
```

> `lark_sheets_csv_put` 比 `lark_sheets_cells_set` 短得多——只想批量灌纯值时优先用它。需要公式/样式才换 `lark_sheets_cells_set`。
>
> ⚠️ `=` 开头的字符串会被当字面量写入（**不会变公式**）：
>
> ```
> lark_sheets_csv_put(url="...", sheet_name="Sheet1", start_cell="A1", csv="name,score\nalice,=SUM(B2:B10)")
> # ↑ A2 实际写入字符串 "=SUM(B2:B10)"，**不是公式**。需要写公式请用 `lark_sheets_cells_set`。
>
```
...
> - dry-run 与成功响应都回显 `writes_range`（实际落区，如 `B2:D4`）：**写前先 `dry_run` 看一眼落区**，确认不会盖到相邻数据。
> - 要保护非空 cell：`allow_overwrite=false`（落区内出现非空 cell 即报错）。

### Validate / DryRun / Execute 约束

- `Validate`：XOR 公共四件套；`lark_sheets_cells_set` 的 `cells` 必须能解析为 JSON 二维矩阵且行列数与 `range` 完全一致；`lark_sheets_cells_set_style` 的样式 flag 至少一个非空（或带 `border_styles`）；`lark_sheets_cells_set_image` 的 `range` 必须是单 cell（起止 cell 相同）；`lark_sheets_csv_put` 的 `csv` 必须能按 RFC 4180 解析；防爆参数上限校验。
