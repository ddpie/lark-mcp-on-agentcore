...
- `start_revision` 是复核基线，即你认定的"编辑前"版本。
- `end_revision` 是"编辑后"版本；**省略时默认取最新 revision**，返回从 start 到最新的全部 changeset。
...
| 工具 | Risk | 分组 |
| --- | --- | --- |
| `lark_sheets_changeset_get` | read | 变更记录 |

## 参数

### `lark_sheets_changeset_get`

_公共：URL/token（无 sheet 定位）_

| 参数 | Type | 必填 | 说明 |
| --- | --- | --- | --- |
| `start_revision` | int | required | 起始版本（编辑前基线，>= 1） |
| `end_revision` | int | optional | 结束版本（省略取最新） |
...
- 最外层 `latest_revision` 是**当前表格的最新版本号**（与查询区间无关），便于判断表格当前停在哪个版本、`start_revision` 该取多少。
...
1. 记下 AI 开始编辑前的 revision（编辑前 `lark_sheets_workbook_info` 或上一次工具返回的 revision 即可作为 `start_revision`）。
2. AI 编辑完成后，调 `lark_sheets_changeset_get(url="<表格>", start_revision=<编辑前版本>)`（不传 end → 取到最新）。
...
- `lark_sheets_changeset_get` 是**只读**操作，不改动表格。
...
公共：`url` / `spreadsheet_token`（二选一，无 sheet 定位）。changeset 是工作簿级历史，不接受 sheet 定位参数。

示例：

```
# 只传起始版本 → 返回从该版本到最新的全部 changeset（最常用：复核 AI 编辑前后的差异）
lark_sheets_changeset_get(url="https://example.feishu.cn/sheets/shtXXX", start_revision=120)

# 传起始 + 结束版本（版本差 end-start+1 ≤ 20）
lark_sheets_changeset_get(spreadsheet_token="shtXXX", start_revision=120, end_revision=135)
```

输出契约（envelope.data）：

- `latest_revision` — 当前表格最新版本号（与查询区间无关）
- `start_revision` / `end_revision` — 实际查询区间（省略 `end_revision` 时 `end_revision` = 最新版本）
- `changesets[]` — 按版本顺序排列；每项含 `revision` / `create_time` / `actions`（原始操作列表）/ `is_self_edit` / `is_ai_edit`

### Validate / Execute 约束

- 校验阶段只做 XOR 检查（`url` / `spreadsheet_token` 二选一）与版本上限校验（`start_revision ≥ 1`，传了 `end_revision` 时 `end ≥ start` 且 `end - start + 1 ≤ 20`）。
- 实际执行才发起 changeset 查询；省略 `end_revision` 时由服务端解析为最新 revision。
