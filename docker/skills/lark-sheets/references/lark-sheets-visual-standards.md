...
- **列宽 / 行高调整**（飞书 `lark_sheets_cols_resize` / `lark_sheets_rows_resize` 直接给像素值：统一尺寸用 `range` + `width`/`height`（像素），多列 / 多行不同尺寸用 `widths`/`heights` map 一次原子完成，如 `widths={"A":100,"C:E":120}`）：
...
Step 3 — 微调收尾：`lark_sheets_rows_resize(heights=…)` / `lark_sheets_cols_resize(widths=…)`（行高列宽 map 一次原子完成）、`lark_sheets_batch_update` + `lark_sheets_cells_{merge|unmerge}` 等
