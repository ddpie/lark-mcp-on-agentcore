...
当用户要求"画个图 / 数据可视化 / 趋势图 / 对比图 / 占比图"时，**必须**通过 `lark_sheets_chart_{create|update|delete}` 创建真实的图表对象。**禁止**用本地脚本调 matplotlib / seaborn 生成图片再插入到表格代替——静态图片无法随源数据更新，且失去交互能力。判断标准：交付后 `lark_sheets_chart_list` 必须能返回该对象。
...
| 创建/更新/删除图表 | `lark_sheets_chart_{create|update|delete}` | 对图表对象执行写入操作 |
...
- **axes[].label 不接受 `format` / `number_format` 字段**：想给坐标轴数值加千分位、百分号等格式化时，不要在 `axes[i].label` 里传 `format` 或 `number_format`（schema 未定义，会报 `unexpected property "format" is not defined in schema`）。数值格式化统一在源数据单元格的 `cell_styles.number_format` 里设置（写 `lark_sheets_cells_set` 时），图表会沿用单元格格式。
...
2. 调 `lark_sheets_csv_get(sheet_id, 'A1:E30')` 或 `lark_sheets_pivot_list` 读 pivot 产物的**实际数据范围**
3. 识别并排除"总计"/"小计"行（通常最后一行；嵌套 pivot 还要排除中间层小计）
4. `lark_sheets_chart_create(create)` 时 `snapshot.data.refs` 精确到数据行（如 pivot 占 A1:D9、总计在 row9 → chart 用 `A1:D8`）
...
4. **不够就先扩表**，二选一，禁止硬塞越界位置：
   ...
   - 否则先调 `lark_sheets_dim_insert`（`lark_get_skill(domain="sheets", section="sheet-structure")`）扩行/列，再 create。
...
| Flag | Type | 必填 | 说明 |
| --- | --- | --- | --- |
| `properties` | string + File + Stdin（复合 JSON） | required | 图表完整配置 JSON。顶层字段为 `position` / `offset` / `size` / `snapshot`（无顶层 `data`，也无再嵌一层 `properties`）；图表数据配置在 `snapshot.data` 下（含 `refs` / `headerMode` / `dim1` / `dim2`）。结构嵌套深，完整结构用 `lark_discover(query="sheets.chart-create")` 查看 |
...
| `properties` | string + File + Stdin（复合 JSON） | required | 完整或足够完整的图表配置 JSON（先 `lark_sheets_chart_list` 回读再 patch） |
...
公共四件套：所有 shortcut 顶部排列 `url` / `spreadsheet_token` / `sheet_id` / `sheet_name`（XOR 规则同 `lark_sheets_csv_get`）。
...
```
lark_sheets_chart_create(url="https://example.feishu.cn/sheets/shtXXX", sheet_name="Sheet1", properties={"position":{"row":42,"col":"A"},"size":{"width":600,"height":400},"snapshot":{"data":{"refs":[{"value":"'Sheet1'!A1:B10"}],"dim1":{"serie":{"index":1}},"dim2":{"series":[{"index":2}]}},"plotArea":{"plot":{"type":"column"}}}})

# 走文件（推荐配置较多时）
lark_sheets_chart_create(url="...", sheet_name="Sheet1", properties="@chart-config.json")
```
...
```
lark_sheets_chart_create(url="...", sheet_name="Sheet1", properties={"position":{"row":24,"col":"F"},"size":{"width":600,"height":450},"snapshot":{"title":{"text":"各部门员工人数占比"},"plotArea":{"plot":{"type":"pie","series":[{"index":1,"sectors":{"sector":[{"index":1,"offsetRadius":0.05}]}}]}},"data":{"refs":[{"value":"'Sheet1'!A1:B11"}],"dim1":{"serie":{"index":1,"aggregate":true}},"dim2":{"series":[{"index":2,"aggregateType":"sum"}]}}}})
```
...
3. 把**完整 snapshot** 整个回写到 `properties` 的 `snapshot` 字段

```
lark_sheets_chart_update(url="...", sheet_id="$SID", chart_id="chrXXX", properties={"position":{"row":0,"col":"A"},"size":{"width":480,"height":320},"snapshot":"<完整快照（由 lark_sheets_chart_list 取回后局部修改）>"})
```
...
lark_sheets_chart_delete(url="https://example.feishu.cn/sheets/shtXXX", sheet_id="$SID", chart_id="chrXXX")
...
- `Validate`：XOR 公共四件套；`lark_sheets_chart_create` / `lark_sheets_chart_update` 的 `properties` 必须能解析为合法 JSON；`lark_sheets_chart_delete`（high-risk-write）校验 `yes` 或 `dry_run` 至少一个。
- `DryRun`：`lark_sheets_chart_create` / `lark_sheets_chart_update` 输出"将要 POST 的 body 模板"；`lark_sheets_chart_delete` 输出"将要删除的 chart_id 及隶属 sheet"，零网络副作用。
- `Execute`：写操作执行后不自动回读；如需确认，自行调用 `lark_sheets_chart_list` 比对结果。

> `lark_sheets_chart_create` / `lark_sheets_chart_update` 是 write 级别，按需可用 `dry_run` 预览，不要求 `yes`。只有 `lark_sheets_chart_delete`（high-risk-write）必须 `yes`。
