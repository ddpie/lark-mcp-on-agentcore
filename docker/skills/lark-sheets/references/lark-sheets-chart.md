...
| 参数 | Type | 必填 | 说明 |
| --- | --- | --- | --- |
| `properties` | object（复合 JSON） | required | 图表完整配置 JSON。顶层字段为 `position` / `offset` / `size` / `snapshot`（无顶层 `data`，也无再嵌一层 `properties`）；图表数据配置在 `snapshot.data` 下（含 `refs` / `headerMode` / `dim1` / `dim2`）。结构嵌套深，完整结构用 `lark_discover(query="sheets.chart_create")` 查看 |
