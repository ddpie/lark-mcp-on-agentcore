...
| `+dashboard-block-get-data` | 获取图表组件的计算结果 | `lark_get_skill(domain="base", section="dashboard-block-get-data")`、`lark_get_skill(domain="base", section="dashboard")`、`lark_get_skill(domain="base", section="dashboard-block-data-config")` | 适合读取图表组件的最终计算结果；此命令不返回 block 元数据，只返回计算结果 |
