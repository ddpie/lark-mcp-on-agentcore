...
| 参数 | 必填 | 说明 |
|------|------|------|
| `share_token <token>` | 是 | 表单分享 Token（从表单分享链接中提取） |
...
```
lark_base_form_detail(share_token="shrbcvST8eZy0vk8zjVZ1CAXNye")
```
...
| `title` | string | 是 | 题目标题 |
| `type` | string | 是 | 字段类型（见下方类型对照表，与 `lark_get_skill(domain="base", section="shortcut-field-properties")` 对齐） |
...
除上述固定字段外，每种 `type` 还会携带该类型特有的配置字段（与 `lark_get_skill(domain="base", section="shortcut-field-properties")` 中的「常见补充字段」对应），例如：
...
与 `lark_get_skill(domain="base", section="shortcut-field-properties")` 中的字段类型完全对齐。
...
- `lark_get_skill(domain="base", section="form-submit")` — 获取详情后可用 submit 填写提交
