...
```
# 列出全部工作流
lark_base_workflow_list(base_token="BascXxxxxx")

# 只看已启用的工作流
lark_base_workflow_list(base_token="BascXxxxxx", status="enabled")

# 只看已禁用的工作流
lark_base_workflow_list(base_token="BascXxxxxx", status="disabled")
```
...
| `page_size <n>` | 否 | 每页大小，默认 100，最大 100 |
...
- `lark_get_skill(domain="base", section="workflow-disable")` — 禁用工作流
