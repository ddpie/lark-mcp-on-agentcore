...
2. **阅读 `lark_get_skill(domain="base", section="workflow-guide")`**，获取 Loop、IfElseBranch、SwitchBranch 等**完整示例**（与创建场景共用相同的步骤结构）
...
| `workflow_id <id>` | 是 | 工作流 ID（`wkf` 开头），可从 `+workflow-list` 获取 |
...
- `lark_get_skill(domain="base", section="workflow-schema")` — Workflow 数据结构参考
- `lark_get_skill(domain="base", section="workflow-guide")` — **完整示例、构造技巧、常见错误**
- `lark_get_skill(domain="base", section="workflow-create")` — 创建工作流
