...
> 💡 **按需查阅：** 如需深入理解返回的 `steps` 节点结构，可参考 `lark_get_skill(domain="base", section="workflow-schema")`。简单统计（如节点数量）无需阅读 schema。
...
```
# 基本用法
lark_base_workflow_get(base_token="BascXxxxxx", workflow_id="wkfxxxxxx")

# 指定用户 ID 类型（creator_id / updater_id 字段的格式）
lark_base_workflow_get(base_token="BascXxxxxx", workflow_id="wkfxxxxxx", user_id_type="open_id")
```
...
| 参数 | 必填 | 说明 |
|------|------|------|
| `base_token <token>` | 是 | 多维表格 Base Token，以 `Basc` 开头 |
| `workflow_id <id>` | 是 | Workflow ID，以 `wkf` 开头 |
| `user_id_type <type>` | 否 | 控制 `creator_id` / `updater_id` 字段返回的用户 ID 格式；枚举值：`open_id`（默认）、`union_id`、`user_id` |
...
|------|------|------|
| `id` | string | 步骤唯一 ID |
| `type` | string | 步骤类型，见 `lark_get_skill(domain="base", section="workflow-schema")` |
| `title` | string | 步骤标题 |
...
| `data` | object | 步骤详细配置，结构随 `type` 变化，见 `lark_get_skill(domain="base", section="workflow-schema")` |
| `meta` | object | 扩展元信息，仅 `APIHubAction` / `AIAgentLLMAction` / `AIAgentMCPAction` 等子节点有值 |

> 步骤类型（`type`）的完整枚举和每种类型的 `data` 字段结构，参见 `lark_get_skill(domain="base", section="workflow-schema")`。
...
- `lark_get_skill(domain="base", section="workflow-schema")` — Workflow 步骤数据结构完整参考
- `lark_get_skill(domain="base", section="workflow-list")` — 列出所有 workflow（可用来获取 workflow_id）
- `lark_get_skill(domain="base", section="workflow-create")` — 创建 workflow
- `lark_get_skill(domain="base", section="workflow-update")` — 更新 workflow
