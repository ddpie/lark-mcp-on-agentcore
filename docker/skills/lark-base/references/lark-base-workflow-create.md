...
2. **阅读 `lark_get_skill(domain="base", section="workflow-guide")`**，获取 Loop、IfElseBranch、SwitchBranch 等**完整示例**
3. **参考 `lark_get_skill(domain="base", section="workflow-schema")`**，查询具体字段定义
...
| 参数 | 必填 | 说明 |
|------|------|------|
| `base_token <token>` | 是 | 多维表格 Base Token（`Basc` 开头） |
...
> **步骤数据结构非常复杂，详见 `lark_get_skill(domain="base", section="workflow-schema")`**
...
- ⚠️ **权限不足**：如遇 `permission denied`，先确认当前用户是否对该 Base 有编辑权限，再检查 scope 是否已开通（MCP server 自动处理认证）
...
- `lark_get_skill(domain="base", section="workflow-schema")` — 完整 Workflow 数据结构
- `lark_get_skill(domain="base", section="workflow-update")` — 全量更新工作流
- `lark_get_skill(domain="base", section="workflow-enable")` — 启用工作流
- `lark_get_skill(domain="base", section="workflow-list")` — 列出全部工作流
