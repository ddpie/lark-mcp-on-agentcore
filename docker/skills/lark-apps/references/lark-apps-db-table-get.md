...
用于查看已知表的字段、索引、约束，或给 SQL/迁移生成提供依据。只想知道有哪些表时先 `lark_apps_db_table_list`。
...
lark_apps_db_table_get(app_id="app_xxx", table="orders", env="dev", format="pretty")
```

## 输出契约

- 默认 JSON 读取 `data.name`、`columns`、`indexes`、`constraints`、`estimated_row_count`、`size_bytes`。
- `format="pretty"` 输出是服务端返回的 DDL 文本，不是 JSON envelope；需要建表语句时可原样给用户。
