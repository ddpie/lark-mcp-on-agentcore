...
- ✅ 已知一组 open_id 想批量校验或回填字段(`user_ids`,最多 100,支持 `me`)
- ✅ 按聊天关系 / 在职状态 / 租户边界 / 企业邮箱等维度筛选员工
- ❌ 已知 open_id 想拿完整 profile → 用 `lark_contact_get_user`（⚠️ bot 身份不可用）
- ❌ 已知 open_id 想发消息 → 直接走 `lark_im`,不经过本工具
...
# 按完整邮箱搜(命中通常唯一,适合作后续工具的输入)
lark_contact_search_user(query="alice@example.com")

# 查看自己
lark_contact_search_user(user_ids="me")

# 批量回填:已知一组 open_id,取姓名 / 邮箱 / 部门
lark_contact_search_user(user_ids="ou_a,ou_b,ou_c", format="json")

# 多 filter 组合:同租户的、有企业邮箱的「王」姓员工
lark_contact_search_user(query="王", exclude_external_users=true, has_enterprise_email=true)

# filter-only 枚举:列出所有"聊过天的离职同事"(无关键词)
lark_contact_search_user(has_chatted=true, left_organization=true)
```
...
```
lark_contact_search_user(queries="Alice,Bob,张三")
```

- 每行 user 带 `matched_query`,标识来自哪个 query
- `queries[]` 每个输入一条 `{query, error?, has_more}`,失败的有 `error`
- 部分失败不影响其它 query;全部失败才报错

```
# bool filter 对每个 query 都生效
lark_contact_search_user(queries="Alice,Bob", has_chatted=true)

# 与 query / user_ids 互斥
lark_contact_search_user(queries="a", query="b")   # ❌ 报错
```
...
| 字段 | 类型 | 说明 | 跨租户 |
|---|---|---|---|
| `open_id` | string | 稳定标识,后续工具的输入 | 始终非空 |
...
| `p2p_chat_id` | string | 与当前用户的 P2P 会话 ID(`oc_...`);空表示从未私聊过。可作为 IM 工具的 `chat_id` 输入 | 可能为空 |
