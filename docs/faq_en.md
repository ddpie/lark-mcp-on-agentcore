...
A: Re-run `./scripts/deploy.sh`. CDK rebuilds the Docker image with the latest lark-cli. Use `./scripts/check-lark-cli-version.sh` to verify the version actually changed. Existing users are unaffected.
